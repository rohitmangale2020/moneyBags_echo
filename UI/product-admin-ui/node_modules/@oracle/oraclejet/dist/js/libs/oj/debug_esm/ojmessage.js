...
import { isTouchSupported, getReadingDirection, validateURL } from 'ojs/ojdomutils';
...
MessageViewModel.prototype._playSound = function (sound) {
  ...
  if (sound !== 'defaults') {
    try {
      validateURL(sound, ALLOWED_AUDIO_URL_PROTOCOLS);
    } catch (error) {
      info(`JET oj-message: Invalid URL in message.sound='${sound}'. Error: ${error}`);
      return;
    }
