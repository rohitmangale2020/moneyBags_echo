...
function _startLiveReloadServer(opts, port, context) {
  return new Promise((resolve, reject) => {
    ...
    const hostname = _getLiveReloadHostname(context);
    if (server && server.listen && livereload) {
      ...
      server.listen(port, hostname, (err) => {
        ...
        util.log(`Listening on ${hostname}:${port}.`);
...
function _getLiveReloadHostname(context) {
  const connectOpts = context.connectOpts || {};
  const hostname = connectOpts.hostname;
...
function _runCustomPostWatchPromise(context) {
  ...
  util.log(`Trigggering commands ${customCommands.map(_formatCommandForLog).join(', ')}.`);
  customCommands.forEach((command) => {
    commandSequence = commandSequence.then(() => _getCommandPromise(command))
      .then(() => {
        util.log(`Command ${_formatCommandForLog(command)} completed..`);
      });
  });
  return commandSequence;
}
...
function _getCommandPromise(command) {
  ...
  if (typeof command === 'string') {
    return Promise.reject(
      new Error(
        `Unknown watch command '${command}' is not allowed. ` +
        'Use a built-in command name or a structured command object like { cmd, args }.'
      )
    );
  }

  const validationError = _validateSpawnCommand(command);
  if (validationError) {
  ...
  return util.spawn(
    ...
    undefined,
    undefined,
    _getSpawnOptions(command)
  );
}

function _validateSpawnCommand(command) {
  if (!command || typeof command !== 'object' || Array.isArray(command)) {
    return `Invalid watch command '${_formatCommandForLog(command)}'.`;
  }

  const supportedKeys = new Set(['cmd', 'args', 'cwd', 'env']);
  const unsupportedKey = Object.keys(command).find(key => !supportedKeys.has(key));
  if (unsupportedKey) {
    return `Unsupported watch command property '${unsupportedKey}'.`;
  }

  if (typeof command.cmd !== 'string' || command.cmd.trim() === '') {
    return 'Watch command property \'cmd\' must be a non-empty string.';
  }

  if (command.args !== undefined &&
    (!Array.isArray(command.args) || command.args.some(arg => typeof arg !== 'string'))) {
    return 'Watch command property \'args\' must be an array of strings.';
  }

  if (command.cwd !== undefined && typeof command.cwd !== 'string') {
    return 'Watch command property \'cwd\' must be a string.';
  }

  if (command.env !== undefined &&
    (!command.env || typeof command.env !== 'object' || Array.isArray(command.env) ||
      Object.keys(command.env).some(key => typeof command.env[key] !== 'string'))) {
    return 'Watch command property \'env\' must be an object with string values.';
  }

  return null;
}

function _getSpawnOptions(command) {
  const spawnOptions = {};
  ...
  return Object.keys(spawnOptions).length > 0 ? spawnOptions : undefined;
}

function _formatCommandForLog(command) {
  if (typeof command === 'string') {
    return command;
  }
  if (command && typeof command === 'object' && !Array.isArray(command)) {
    const args = Array.isArray(command.args) ? command.args : [];
    return [command.cmd].concat(args).filter(Boolean).join(' ');
  }
...
function _isThemeFile(filePath) {
  const srcThemes = new RegExp(util.escapeForRegExp(config('paths').src.themes));
  const stagingThemes = new RegExp(util.escapeForRegExp(config('paths').staging.themes));
  const srcPath = new RegExp(util.escapeForRegExp(config('paths').src.common));
...
module.exports.__getCommandPromise = _getCommandPromise;
module.exports.__startLiveReloadServer = _startLiveReloadServer;
module.exports.__getLiveReloadHostname = _getLiveReloadHostname;
module.exports.__isThemeFile = _isThemeFile;
