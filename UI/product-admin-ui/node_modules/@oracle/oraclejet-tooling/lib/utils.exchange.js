...
let requestObject = {};
let accessTokenMapCache = null;

function encodeExchangePathSegment(value) {
  return encodeURIComponent(String(value));
}

function getExchangeQueryString(params) {
...
exchangeUtils.getComponentMetadata = function (componentName) {
  ...
  const pathBase = `/components/${encodeExchangePathSegment(plainComponentName)}`;
  const requestPath = requestedVersion ?
    `${pathBase}/versions/${encodeExchangePathSegment(requestedVersion)}` : pathBase;
...
exchangeUtils.readAuthInfoFromFS = () => {
  ...
  if (accessTokenMapCache && accessTokenMapCache[exchangeUrl]) {
    return exchangeUtils.isAuthTokenExpired(accessTokenMapCache[exchangeUrl]) ?
      {} : accessTokenMapCache[exchangeUrl];
  }
  ...
  if (accessTokenMap && accessTokenMap[exchangeUrl]) {
    ...
    // Cache in this process only so child processes do not inherit tokens.
    accessTokenMapCache = accessTokenMap;
...
exchangeUtils.uploadToExchange = function (componentOrPackName, options) {
  return new Promise((resolve, reject) => {
    const opts = options || {};
    const uploadPath = `/components/${opts._batchUpload ? '_batchUpload' : ''}`;
    const queryParams = { access: 'PUBLIC' };
    if (opts._batchUpload) {
      queryParams.onDuplicates = 'warn';
    }
    if (options.labels) {
      queryParams.labels = options.labels.replaceAll(/\s/g, '');
    }
    const requiredPath = `${uploadPath}?${getExchangeQueryString(queryParams)}`;
...
exchangeUtils.writeAuthInfoToFS = (authResponse) => {
  ...
  const accessTokenMap = accessTokenMapCache ||
    util.readJsonAndReturnObject(pathToFile, { suppressNotFoundError: true }) || {};
  const exchangeUrl = exchangeUtils.getExchangeUrl();
  /**
  ...
  // Tighten an existing token file before truncating/writing new credentials.
  if (fs.existsSync(pathToFile)) {
    fs.chmodSync(pathToFile, 0o600);
  }

  // Write file with owner-only permissions and correct any existing broader mode.
  util.writeObjectAsJsonFile(pathToFile, accessTokenMap, { mode: 0o600 });
  fs.chmodSync(pathToFile, 0o600);

  // Cache new map in this process only so child processes do not inherit tokens.
  accessTokenMapCache = accessTokenMap;
};
...
exchangeUtils.listComponentsWithGivenLabels = function (options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const query = getExchangeQueryString({
      format: 'compact',
      q: `label:${options.labels}.json`
    });
    util.request({
      useUrl: `${exchangeUrl}/components?${query}`,
      method: 'GET',
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json'
...
exchangeUtils.addComponentLabel = function (component, label) {
  ...
  const encodedName = encodeExchangePathSegment(name);
  const encodedVersion = encodeExchangePathSegment(version);
  return new Promise((resolve, reject) => {
    util.request({
      method: 'POST',
      headers: {
      ...
      useUrl: `${exchangeUrl}/components/${encodedName}/versions/${encodedVersion}/labels`,
    }, JSON.stringify({
      name: `${label}`,
...
exchangeUtils._encodeExchangePathSegment = encodeExchangePathSegment;
exchangeUtils._getExchangeQueryString = getExchangeQueryString;
