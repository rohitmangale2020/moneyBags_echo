 ...
 * @param {boolean} [options.directoryListing=false] - Enable directory listing for served files
...
function _validateOptions(options, validPlatform) {
  ...
  validOptions = _validateDirectoryListing(validOptions);
...
function _getConnectOptions(opts, defaultOpts) {
  ...
  const connectOverrides = opts.connect || {};
  const connectOptionOverrides = connectOverrides.options ||
    connectOverrides;
  const livereload = opts.livereload ? opts.livereloadPort : false;
  let open = connectOptionOverrides.open === undefined
    ? connectConfig.options.open
    : connectOptionOverrides.open;
  open = opts.destination === 'server-only' ? false : open;
  const port = opts.port || connectConfig.options.port;
  const directoryListing = opts.directoryListing !== undefined ?
    opts.directoryListing :
    connectConfig.options.directoryListing;
  const hostname = _normalizeHostname(connectOptionOverrides.hostname ||
    connectConfig.options.hostname);

  if (_isNonLocalHostname(hostname)) {
    util.log.warning(
      `Serving on non-local host '${hostname}'. Ensure this is intentional before exposing your development server.`
    );
  }

  if (directoryListing) {
    util.log.warning('Directory listing is enabled for the development server.');
  }

  return _overrideConnectOptions(connectConfig, {
    livereload,
    open,
    port,
    directoryListing,
...
function _validateDirectoryListing(options) {
  const validOptions = util.cloneObject(options);
  if (validOptions.directoryListing !== undefined) {
    util.validateType('directoryListing', validOptions.directoryListing, 'boolean');
  } else {
    validOptions.directoryListing = config('defaultServeConfig').connect.options.directoryListing;
  }
  return validOptions;
}
...
function _isNonLocalHostname(hostname) {
...
module.exports.__getConnectOptions = _getConnectOptions;
