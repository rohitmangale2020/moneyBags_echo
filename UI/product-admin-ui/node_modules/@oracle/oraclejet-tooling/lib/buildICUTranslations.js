...
function _executeTranslationCommand(l10nBundleBuilder, args) {
  // eslint-disable-next-line arrow-body-style
  return new Promise((resolve, reject) => {
    return execFile(process.execPath, [l10nBundleBuilder, ...args], (error) => {
      if (error) {
 ...
 *
 * @returns {Array} The generated command arguments for building translations.
 */
function _generateICUTranslationArgs(configObj) {
  const {
    translationOptions,
    configuredOptions
  } = configObj;
  ...
  if (configuredOptions.length > 0) {
    configuredOptions.forEach((key) => {
      if (translationOptions[key]) {
        ...
        const value = (key === constants.TRANSLATION_OPTIONS.SUPPORTED_LOCALES_KEY) ?
          translationOptions[key].replace(/\s+/g, '') :
          translationOptions[key];
        args.push(`--${key}=${value}`);
      }
    });
...
async function _buildICUTranslationsBundleForComponent(pathToResourcesFolder) {
  try {
    ...
    if (fs.existsSync(bundleFile)) {
      ...
      const args = _generateICUTranslationArgs({
        translationOptions,
        configuredOptions
      });
      ...
      // Execute the command
      await _executeTranslationCommand(l10nBundleBuilder, args);
