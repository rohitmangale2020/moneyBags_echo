...
app.addTesting = function (options) {
  ...
  return toolingModule.addtesting(options)
    ...
    .then(() => {
      if (options[constants.VIEW_MODELS]) {
        _addViewModelsConfigurationsToKarmaConfigObject();
        _updateTestMainJs();
        _updateTsConfig();
      }
    })
...
app.restore = function (options) {
  // Store restore options in this process only so child processes do not inherit credentials.
  restoreOptions = options ? Object.assign({}, options) : {};
...
function _validateOjetConfigFile(pathToApp) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (!fileDescription) {
      ...
      } else if (removeWhiteSpaces(constants.OJET_CONFIG_FILE_DESCRIPTION) !==
        ...
        utils.log.warning(`Description mismatch detected in ojet.config.js. Please update it to match the standard format: ${constants.OJET_CONFIG_FILE_DESCRIPTION}.`);
      }

      // Validate function return type and contents using AST parsing
      const requiredProperties = ['webpack', 'context'];
      let returns;
      try {
        returns = utils.loadToolingUtil().collectReturnStatements(fileContent);
      } catch (parseError) {
        // If AST parsing fails, log a single WARNING and skip strict shape checks
        utils.writeToMigrationLogFile(
          'WARNING',
          'Skipping strict validation of ojet.config.js due to a parsing issue. Please ensure the file syntax is valid.'
        );
        utils.log.warning('Skipping strict validation of ojet.config.js due to a parsing issue.');
      ...
      if (returns.length === 0) {
        utils.writeToMigrationLogFile(
          'ERROR',
          'No return statement found in the webpack function in ojet.config.js.'
        );
        utils.log.error('No return statement found in the webpack function in ojet.config.js.');
      } else {
        // Get all object returns
        const objectReturns = returns.filter(ret => ret.type === 'object' && ret.properties);

        if (objectReturns.length === 0) {
          utils.writeToMigrationLogFile(
            'ERROR',
            'The webpack function in ojet.config.js must return an object. Expected "return { context, webpack: config }".'
          );
          utils.log.error('The webpack function in ojet.config.js must return an object. Expected "return { context, webpack: config }".');
        } else {
          // Check that ALL object returns have the required properties
          objectReturns.forEach((ret, index) => {
            const missingProperties = requiredProperties.filter(
              prop => !ret.properties.includes(prop)
            );

            if (missingProperties.length > 0) {
              const errorMessage = objectReturns.length > 1
                ? `Return statement ${index + 1} in ojet.config.js is missing required properties: ${missingProperties.join(', ')}. Expected "return { context, webpack: config }".`
                : `The returned object from the webpack function in ojet.config.js is missing required properties: ${missingProperties.join(', ')}. Expected "return { context, webpack: config }".`;

              utils.writeToMigrationLogFile('ERROR', errorMessage);
...
function _runAfterAppRestoreHook() {
  return new Promise((resolve, reject) => {
    ...
    const hookPath = commonHookRunner.__resolveHookPath(hooksConfig.after_app_restore);
...
function _renameTestTemplatePrefixFile({ pathToSpecsFolder, componentsPath, pack, component }) {
  ...
  testFiles.forEach((file) => {
    if (/@component@/.test(file)) {
      ...
      let fileContent = fs.readFileSync(renamedPathToTestFile, { encoding: 'utf-8' });
      ...
      // If this is a pack component, prefer pack aliases in Karma TS spec imports
      // Example rewrites after placeholder substitution:
      //   import 'component/loader'             => import 'pack/component/loader'
      //   from "component/component-viewModel" => from "pack/component/component-viewModel"
      if (pack) {
        const aliasBase = `${pack}/${component}`;
        // Rewrite loader imports, preserving original quote style
        const loaderImportRegex = new RegExp(`(["'])${component}/loader\\1`, 'g');
        fileContent = fileContent.replace(loaderImportRegex, (_m, q) => `${q}${aliasBase}/loader${q}`);
        // Rewrite viewModel default import path, preserving quotes
        const vmFromRegex = new RegExp(`from\\s+(["'])${component}/${component}-viewModel\\1`, 'g');
        fileContent = fileContent.replace(vmFromRegex, (_m, q) => `from ${q}${aliasBase}/${component}-viewModel${q}`);
      }

      fs.writeFileSync(renamedPathToTestFile, fileContent);
    }
  });
}
...
function _createInitFile(filePath) {
  ...
  // Prefer using templates from oraclejet-tooling (.tmpl files)
  const isJsRoot = filePath.endsWith('root.js');
  const templateFile = isJsRoot ? 'init-entry.js.tmpl' : 'init-entry.ts.tmpl';
  // Resolve template location using oraclejet-tooling's util.rootPath (no hardcoded module path)
  const toolingUtil = utils.loadToolingUtil();
  const toolingLibRoot = toolingUtil && toolingUtil.rootPath; // points to oraclejet-tooling/lib
  const templatePath = toolingLibRoot ? path.join(toolingLibRoot, 'templates', 'webpack', templateFile) : '';

  if (!toolingLibRoot || !fs.existsSync(templatePath)) {
    utils.log.error(
      'Missing multi-locale init entry template.\n' +
      `Expected at: ${templatePath || '(unresolved)'}\n` +
      'Please ensure @oracle/oraclejet-tooling is installed and up to date, and that\n' +
      `the file templates/webpack/${templateFile} exists under the tooling installation.\n` +
      'Try running \'ojet restore\' or reinstalling dependencies.'
    );
    return;
  }

  const template = fs.readFileSync(templatePath, 'utf-8');
  const generatedContent = template.replace(/__ROOT_MODULE__/g, fileNameWithoutExtension);
  fs.writeFileSync(filePath.replace(fileName, `init-${fileName}`), generatedContent);
}
...
function _addViewModelsConfigurationsToKarmaConfigObject() {
  const configPaths = utils.loadToolingUtil().getConfiguredPaths();
  const pathToViewModels = utils.getPathToViewModelsInSrc();
  const pathToKarmaConfigFile = utils.getPathToKarmaConfigFile();

  if (!fs.existsSync(pathToViewModels)) {
    utils.log.error('Unable to locate viewModels folder in your project. Expected to find it at src/{ts | js}/viewModels (or src/<custom-folder>/viewModels if ts/js folders are customized), but it was not found. Please ensure that the folder exists and try again.');
  }

  let fileContent = fs.existsSync(pathToKarmaConfigFile) ? fs.readFileSync(pathToKarmaConfigFile, { encoding: 'utf-8' }) : '';
  const filesPropertyRegex = /files\s*:\s*\[(.*?)\]/s;
  let match = fileContent.match(filesPropertyRegex);

  if (!match) {
    utils.log.error('Could not find \'files\' array in the config object in your karma.config.js file.');
  }

  const viewModelsPattern = `${configPaths.staging.web}/${configPaths.src.javascript}/viewModels/**/*.js`;
  const accUtilsPattern = `${configPaths.staging.web}/${configPaths.src.javascript}/accUtils.js`;

  let newArrayText;

  if (!fileContent.includes(viewModelsPattern)) {
    newArrayText = `
      {
        pattern: "${viewModelsPattern}",
        included: false
      }
    `;
  }

  if (!fileContent.includes(accUtilsPattern)) {
    newArrayText = `
      ${newArrayText},
      {
        pattern: "${accUtilsPattern}",
        included: false
      }
    `;
  }
  ...
  if (newArrayText) {
    fileContent = fileContent
      .replace(
        filesPropertyRegex,
        `files: [${match[1]}, // viewModels files:${newArrayText}]`
      );
  }

  const preprocessorRegex = /preprocessors\s*:\s*\{(.*?)\}/s;

  match = fileContent.match(preprocessorRegex);

  if (!match) {
    utils.log.error('Could not find \'preprocessors\' property in config object');
  }

  const viewModelsFilePattern = `'${viewModelsPattern}': ['coverage']`;

  if (!fileContent.includes(viewModelsFilePattern)) {
    fileContent = fileContent
      .replace(
        preprocessorRegex,
        `preprocessors: {${match[1]}, \t${viewModelsFilePattern}}`
      );
  }

  fs.writeFileSync(
    pathToKarmaConfigFile,
    fileContent,
    {
      encoding: 'utf-8',
      spaces: 2
...
function _updateTestMainJs() {
  const variableRegex = /var\s*TEST_REGEXP\s*=\s*.*/gi;
  const pathToKarmaConfigFile = utils.getPathToKarmaConfigFile();
  const pathToTestMainJs = pathToKarmaConfigFile.replace('karma.conf.js', 'test-main.js');

  let fileContent = fs.existsSync(pathToTestMainJs) ? fs.readFileSync(pathToTestMainJs, { encoding: 'utf-8' }) : '';
  const newPattern = new RegExp(path.posix.join('__tests__', '.*spec.js$'), 'i');

  fileContent = fileContent.replace(variableRegex, `var TEST_REGEXP = ${newPattern};`);

  fs.writeFileSync(pathToTestMainJs, fileContent, { encoding: 'utf-8', spaces: 2 });
}

function _updateTsConfig() {
  const configPaths = utils.loadToolingUtil().getConfiguredPaths();
  const pathToKarmaConfigFile = utils.getPathToKarmaConfigFile();
  // Since they are in the same level:
  const pathToTsConfig = pathToKarmaConfigFile
    .replace(
      'karma.conf.js',
      'tsconfig.json'
    );
  const tsConfig = utils.readJsonAndReturnObject(pathToTsConfig);
  const srcFolder = configPaths.src.common;
  const tsFolder = configPaths.src.typescript;
  const tsFolderPattern = `../${srcFolder}/${tsFolder}/*`;
  const testFilesPattern = `../${srcFolder}/${tsFolder}/__tests__/*.ts`;

  // eslint-disable-next-line quotes
  if (!tsConfig.compilerOptions.paths["*"].includes(tsFolderPattern)) {
    // eslint-disable-next-line quotes
    tsConfig.compilerOptions.paths["*"].push(tsFolderPattern);
  }

  // eslint-disable-next-line quotes
  if (!tsConfig.compilerOptions.paths["viewModels/*"]) {
    // eslint-disable-next-line quotes
    tsConfig.compilerOptions.paths["viewModels/*"] = [
      // eslint-disable-next-line quotes
      `../${srcFolder}/${tsFolder}/viewModels/*`
    ];
  }

  // eslint-disable-next-line quotes
  if (!tsConfig.compilerOptions.paths["ojs/*"]) {
    // eslint-disable-next-line quotes
    tsConfig.compilerOptions.paths["ojs/*"] = [
      // eslint-disable-next-line quotes
  ...
  // eslint-disable-next-line quotes
  if (!tsConfig.include.includes(testFilesPattern)) {
    // eslint-disable-next-line quotes
    tsConfig.include.push(testFilesPattern);
  }

  fs.writeJsonSync(pathToTsConfig, tsConfig, { encoding: 'utf-8', spaces: 2 });
}
