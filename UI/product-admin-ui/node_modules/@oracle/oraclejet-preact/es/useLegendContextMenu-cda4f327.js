...
import { f as getLegendCurrentItemStyle, h as getItemProps, j as getGridCellIndices, b as getItemInfo } from './useLegendDnd-4848f2a3.js';
...
function ImageMarker(props) {
    ...
    return jsx("img", { class: imageMarkerStyle, src: props.source, role: "none" });
}
...
const BaseLegendItem = forwardRef(({ id, text, sectionIdx, itemIdx, itemId, isHighlighted, symbolHeight, symbolWidth, isFocused, isCurrent, isHidden, actionable = 'inherit', hoverBehavior = 'none', hideAndShowBehavior = 'off', ...props }, ref) => {
    const textStyles = {
...
/**
 * Custom hook to manage datatip (tooltip) display for Legend and Sectional Legend.
 *
 ...
 *
 * @template K - Key type for legend items (string or number)
 * @template D - Legend item data type
 * @template S - Legend section data type
 * @returns {ReturnType<typeof useDatatip>} Datatip configuration for rendering
 */
function useLegendDatatip({ rootDimsRef, focusedItemRef, focusedItemInfo, touchResponse, isDisabled, datatip, items, hoveredItemInfo, sections }) {
    ...
    const activeItemInfo = focusedItemInfo?.isCurrent
        ...
        : hoveredItemInfo?.isCurrent
            ...
            : undefined;
    if (activeItemInfo) {
        const { itemIdx, sectionIdx } = activeItemInfo;
        const activeItem = sections && sections.length > 0
...
const useSectionalLegendContextMenu = ({ rootRef, contextMenuConfig, focusedItemInfo, sections, isReadOnly, onContextMenuDismissed }) => {
    const { contextMenuProps, contextMenuContent } = useContextMenu({
        onContextMenuHandler: ({ gesture, target }) => {
            ...
            const itemIdx = getItemInfo(target)?.itemIdx;
            const sectionIdx = getItemInfo(target)?.sectionIdx;
...
export { BaseLegend as B, LegendText as L, useLegendContextMenu as a, useSectionalLegendContextMenu as b, useLegendDatatip as u };
//# sourceMappingURL=useLegendContextMenu-cda4f327.js.map
