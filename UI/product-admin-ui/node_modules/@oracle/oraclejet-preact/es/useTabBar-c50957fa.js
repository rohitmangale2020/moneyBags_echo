...
import { c as getItemKeysArray, d as getPrevNextKeyUsingRef, k as keyExtractor, f as findElementByKey, a as getKey, b as getFirstVisibleKey } from './collectionUtils-609f73d6.js';
...
import { g as getDefaultContextMenu } from './TabBarContextMenu-d9a31d89.js';
...
function useTabBar(options) {
    ...
    useEffect(() => {
        if (rootRef.current) {
            const tabKeys = getItemKeysArray(rootRef.current, ITEM_SELECTOR);
            if (currentKey == null && tabKeys) {
            ...
            if (currentKey != null && tabKeys && !tabKeys.includes(currentKey)) {
    ...
    const { currentKeyProps } = useCurrentKey((element) => onRemove
        ...
        : keyExtractor(element, ITEM_SELECTOR), false, false, undefined, undefined, getPrevNextKeyUsingRef(rootRef, currentKey, true, ITEM_SELECTOR), getPrevNextKeyUsingRef(rootRef, currentKey, false, ITEM_SELECTOR), currentKey, (detail) => {
    ...
    const { triggerProps } = useContextMenuGesture(({ gesture, anchor, target }) => {
        ...
        if (rootRef.current) {
            tabKeysArray = isReorderable
                ? getItemKeysArray(rootRef.current, ITEM_SELECTOR)
                : undefined;
            isRemovableItem =
...
export { REMOVABLE_ITEM_ATTRIBUTE as R, getItemAttribute as g, useTabBar as u };
//# sourceMappingURL=useTabBar-c50957fa.js.map
