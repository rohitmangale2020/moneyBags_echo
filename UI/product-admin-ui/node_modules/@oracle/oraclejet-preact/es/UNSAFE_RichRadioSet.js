...
import { u as useRadioFocusManager } from './useRadioFocusManager-36eb283e.js';
...
import { B as BaseRichSelection } from './BaseRichSelection-23604860.js';
import 'preact/hooks';
...
import './useContainerBreakpoints-0bc231b7.js';
 ...
 */
const RichRadioSet = forwardRef(({ children, onCommit, value, isDisabled: propIsDisabled, isReadonly: propIsReadonly, ...otherProps }, ref) => {
    const baseId = useId();
    const { isDisabled: isFormDisabled, isReadonly: isFormReadonly } = useFormContext();
    ...
    const { setFocusIndex, handleArrowKeyPress } = useRadioFocusManager(containerRef, children);
    const { focusProps, isFocused } = useFocusWithin({
        isDisabled
    });
    const { isTabbable } = useTabbableMode();
    return (jsx(RadioSetContext.Provider, { value: {
            name: baseId,
            value,
            onCommit,
            setFocusIndex
        }, children: jsx(TabbableModeContext.Provider, { value: { isTabbable: isFocused && isReadonly ? false : isTabbable }, children: jsx("div", { ref: containerRef, onKeyDown: isReadonly ? handleArrowKeyPress : undefined, ...(isReadonly ? focusProps : {}), children: jsx(BaseRichSelection, { ...otherProps, isDisabled: propIsDisabled, isReadonly: propIsReadonly, baseId: baseId, ref: ref, selection: "single", value: value, children: toChildArray(children).map((child, index) => (
                    // index is needed for keyboard navigation with radio items
                    jsx(RadioItemContext.Provider, { value: { index }, children: child }))) }) }) }) }));
});

export { RichRadioSet };
//# sourceMappingURL=UNSAFE_RichRadioSet.js.map
