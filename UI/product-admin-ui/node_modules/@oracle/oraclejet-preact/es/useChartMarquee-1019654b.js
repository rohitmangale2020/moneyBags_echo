...
import { u as useVisDragModeControls } from './useVisDragModeControls-8791a1b4.js';
...
function Marquee({ x1, y1, width, height }) {
    return (jsxs(Fragment, { children: [jsx("rect", { x: x1, y: y1, width: width, height: height, class: styles.visMarquee, "data-oj-private-object": "marquee" }), jsx("rect", { x: x1 + 1, y: y1 + 1, width: Math.max(0, width - 2), height: Math.max(0, height - 2), class: styles.visMarqueeInnerArea })] }));
}
...
function useChartMarquee({ rootRef, plotAreaSpace, xSpace, ySpace, dragMode, chartWidth, isMultiSelection, onViewportChange, isZoomScroll, xScale, selectedIds, getDataById, onSelectionChange, isRtl, xAxisInfo }) {
    ...
    const onPointerDown = (event) => {
        const info = getElementData(rootRef, event.target);
        if (info?.['ojPrivateObject'] === 'bar' || info?.['ojPrivateObject'] === 'item') {
            return;
        }
    ...
    const onPointerMove = (event) => {
        ...
        const elementData = getElementData(rootRef, event.target);
...
//# sourceMappingURL=useChartMarquee-1019654b.js.map
