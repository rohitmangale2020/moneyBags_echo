...
import { u as useTabbableModeSet } from './useTabbableModeSet-cce73d79.js';
import { k as keyExtractor, b as getFirstVisibleKey, f as findElementByKey } from './collectionUtils-609f73d6.js';
...
const LIST_ITEM_SELECTOR = '[data-oj-list-item]';
/**
...
/**
 * Helper method to get the viewport config
 */
const getViewportConfig = (defaultScrollerRef, config) => {
    return config ?? { scrollerRef: defaultScrollerRef };
};
 ...
 *
 * This hook can be shared across ListView, CardView, TreeView. Each component should pass in the item selector.
 */
const useNavigation = (data, rootRef, idExtractor, itemSelector, arrowKeyAndHandlers, currentItemOverride, onPersistCurrentItem, viewportConfig, scrollToVisibleOffset, allowShiftKey, allowTabbableMode, allowActiveDescendant, isEmbedded, allowEnterKey) => {
    ...
    const [isCurrentTabbableKey, tabbableModeProps] = useTabbableModeSet(rootRef, (element) => {
        ...
        return isExpander(element) ? null : keyExtractor(element, itemSelector);
    }, currentKey, onCurrentKeyChange, undefined, undefined, allowEnterKey);
    const { currentKeyProps } = useCurrentKey((element) => keyExtractor(element, itemSelector), allowShiftKey, isCurrentTabbableKey(currentKey), arrowKeyAndHandlers.ArrowUp(currentKey), arrowKeyAndHandlers.ArrowDown(currentKey), arrowKeyAndHandlers.ArrowLeft?.(currentKey), arrowKeyAndHandlers.ArrowRight?.(currentKey), currentKey, onCurrentKeyChange);
    const [showFocusRing, focusRingProps] = useCollectionFocusRing(rootRef, Object.keys(arrowKeyAndHandlers));
    ...
    const handleInvalidCurrentKey = useCallback(() => {
        if (rootRef.current && (!allowTabbableMode || isCurrentTabbableKey(undefined))) {
            const firstKey = pendingCurrentKey.current || getFirstVisibleKey(rootRef.current, itemSelector);
    ...
    const handleFocus = useCallback((event) => {
        ...
        if (rootRef.current) {
            if (!isKeyDefined(currentKey)) {
                ...
                const elem = findElementByKey(rootRef.current, currentKey, itemSelector);
                if (elem) {
    ...
    return {
        navigationProps: mergeProps(currentKeyProps, focusRingProps, allowTabbableMode ? tabbableModeProps : {}, {
            onFocus: handleFocus,
...
export { LIST_ITEM_SELECTOR as L, useNavigation as a, excludeGroup as e, getViewportConfig as g, isMultipleSelection as i, scrollToVisible as s, useCurrentItemOverride as u };
//# sourceMappingURL=useNavigation-ef11265f.js.map
