...
import { dataGridCellStyles } from './PRIVATE_DataGrid/themes/DataGridCell.css.js';
import { dataGridStyles } from './PRIVATE_DataGrid/themes/DataGrid.css.js';
...
function DataGridCell({ cell, height, width }) {
    if (cell.cellType.includes('Skeleton')) {
        return (jsx("div", { style: getCellStyle(cell, width, height), class: getCellClasses(cell), children: jsx(Skeleton, { height: "6x", width: "16x" }) }));
    }
    return (jsx("div", { style: getCellStyle(cell, width, height), class: getCellClasses(cell), children: cell.data }));
}
...
const renderGrid = ({ ranges, totalColumnCount, totalRowCount, onLoadRange, height, width, cell, header }, skeletonList) => {
    ...
    let _actualDataRows = 0;
    let _actualDataCols = 0;
    if (mergedDataBody) {
        mergedDataBody.forEach((range) => {
            _actualDataCols += range.columnCount;
            _actualDataRows += range.rowCount;
        });
    }
...
const DataGrid = function DataGrid(props) {
            ...
            for (let i = topColumnOfViewPort; i < skeletonArray.length; i++) {
                for (let j = topRowOfViewPort; j < skeletonArray[i].length; j++) {
                    if (!skeletonArray[i][j]) {
                        tempSkeletonList.push({ type: 'cellSkeleton', columnIndex: i, rowIndex: j, level: 0 });
                    }
                }
            }
        }
    ...
    return (jsx("div", { id: "dataGrid", role: 'application', class: classes, style: style, children: grid.map((gridCell) => (jsx(DataGridCell, { cell: gridCell, height: cellHeight, width: cellWidth }))) }));
};

export { DataGrid as D };
//# sourceMappingURL=DataGrid-33ac5fa5.js.map
