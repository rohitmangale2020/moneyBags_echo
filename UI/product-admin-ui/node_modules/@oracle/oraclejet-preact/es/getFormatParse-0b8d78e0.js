...
import { F as FormatParseError } from './types-ad8fe56f.js';
...
function getISODateOffset(date, timeZone) {
    ...
    // get the offset of the target timezone (subject to a possible Daylight Savings Time (DST) change adjustment)
    ...
    if (!(date.year < 100)) {
        // shift the date at utc by the offset.
        d.setTime(d.getTime() - offset * 60000);
        const timeZonedDate1 = _applyTimezoneToDate(d, timeZone);
        // If the days and hours are the same then we know that the offset was correct and we do not have to adjust it.
        // If it is not the same, then we are on a Daylight Savings Time (DST) day and hour and we have to figure
        // out which way to adjust the offset.
        if (!_compareDates(timeZonedDate1, date)) {
            adjustment = -60;
            // try to accommodate 'fall back'/move 1 hour west, and see if the hour/day match then.
            d.setTime(d.getTime() + 60 * 60000);
            const timeZonedDate2 = _applyTimezoneToDate(d, timeZone);
            if (!_compareDates(timeZonedDate2, date)) {
                // Next, spring forward/move 1 hour east
                ...
                if (timeZone !== 'Asia/Beirut') {
                    ...
                    // Fix for JET-75436 UNABLE TO SELECT SPECIFICALLY 31.03.2024 DATE FROM CALENDAR IN BEIRUT (EET) TIME ZONE
                    // The 'offset' we calculate with const offset = _getOffset(date, utcDateAtTimezone) cannot be used with adjustment=0
                    // for positive offsets. The formatted date will be off 1 hour.
                    // And we cannot add 60 to it in 'spring forward'. The formatted date will be off 2 hours,
                    // *behind* 2 hours of the date passed in. It should be 1 hour forward.
                    // This fixes the bug where a timezone with a positive offset*
                    // and has DST spring forward does not format to the correct date and time.
                    // *Instead of fixing this for all offset >= 0, we decided to limit the scope to Asia/Beirut which is what
                    // the customer complained about. Filed JET-76400 for DST issues with other timezones.
                    adjustment = -60;
                }
            }
        }
 ...
 */
function isTimeOnlyIsoString(value) {
...
var dateTimeUtils = {
    ...
    _ISO_DATE_ONLY_REGEXP: _ISO_DATE_ONLY_REGEXP,
    isDateOnlyIsoString: isDateOnlyIsoString,
    isTimeOnlyIsoString: isTimeOnlyIsoString,
...
const AM_DESIGNATOR = 'AM';
const PM_DESIGNATOR = 'PM';
class NativeParserImpl {
    static parseImpl(str, pattern, resOptions, cal) {
        ...
        // JET-71233: ISO_DATE_REGEXP will falsely match 4 digit time only values, such
        // as when the user types '1000' into oj-input-time. In this case the time isn't
        // prefaced by 'T' so it gets parsed as a date, which results in '1000' getting
        // parsed as '12:00 AM' instead of '10:00 AM'. To prevent this we check if
        // resOptions says the value is time only but the 'T' is missing, in which
        // case we determine dtStyle using _dateTimeStyle and not __isoStrDateTimeStyle
        // which only looks for the T.
        // Without the _ISO_DATE_ONLY_REGEXP check it is possible we get a date-only string with a time-only options
        // in the converter (e.g., in oj-input-date-time), and think it is a timevalue without a T.
        const isTimeValueWithoutT = !str.includes('T') &&
            this._dateTimeStyle(resOptions) === 1 &&
            !_ISO_DATE_ONLY_REGEXP.test(str);
        let parsedIsoStr = '';
        let isoStrInfo = null;
        let res;
        if (testIsoStr === true && !isTimeValueWithoutT) {
            parsedIsoStr = str;
    ...
    static _parseLenienthms(result, timepart, format, dtype, calNode) {
        // date+times formats in some locales have a ', ' between the date and the time, so remove that if it is there.
        timepart = timepart.startsWith(',') ? timepart.slice(1).trim() : timepart.trim();
        const timeToParse = NativeParserImpl._formatTimeWithColons(timepart, format, calNode);
        ...
        let msec = 0;
        let idx;
        let match = NativeDateTimeConstants._TIME_REGEXP.exec(timeToParse);
        let range;
        if (match === null) {
        ...
        switch (match[0]) {
            case 'h':
                ...
                idx = this._matchPMSymbol(calNode, timeToParse);
                if (idx && hour < 12) {
                    hour += 12;
                }
                break;
    ...
    // lenient parse yMd and yMEd pattern. Must have year, month,
    ...
    static _matchPMSymbol(cal, matchGroup) {
        ...
        if (NativeDateTimeConstants._zh_tw_locales.includes(loc)) {
            ...
            const upperMatchGroupNoSpace = NativeParserImpl._removeSpaces(toUpper(matchGroup));
            const upperCalPMNoSpace = NativeParserImpl._removeSpaces(toUpper(calPM));
            // We leniently match on the locale's 'pm' string, and if that doesn't match then English PM_DESIGNATOR.
            ...
            const exactMatchWithEnglishPM = upperMatchGroupNoSpace.indexOf(PM_DESIGNATOR) !== -1;
    ...
    static _parseExact(value, format, resOptions, cal) {
        ...
        // Fix bug JET-68621 and JET-72563 Unable to set PM time in a site with language as ES
        // 'es''s am/pm is a. m. and p. m.
        // Changing the NativeDateTimeConstants from _WORD_REGEXP = '(\\D+?\\s*)' to  _WORD_REGEXP = '(\\D+\\s*)';
        // fixes this bug, but breaks the parsing of 'ar-SA' locale in 'full' format for the morning times and our converterlang tests failed.
        // So instead change a. m. to AM and p. m. to PM.
        const matchValueAmPm = value
            .replace(/a\.?\s*m\./gi, AM_DESIGNATOR)
            .replace(/p\.?\s*m\./gi, PM_DESIGNATOR);
        const match = new RegExp(parseInfo.regExp).exec(matchValueAmPm);
        if (match === null) {
    ...
    /**
     * Formats a time string that does not have separators
     * by inserting colons as separators.
     *
     * For lenient parsing without separators or fractional seconds,
     * this function inserts a separator every 2 digits from the end
     * to produce the correct grouping of hour/minute/seconds.
     *
     * E.g. '100' → '1:00', '1000' → '10:00', '10000' → '1:00:00'.
     *
     * @private
     * @static
     * @param {string} timeStr - The input time string to be formatted.
     * @param {string} format - The format string used to determine if the time string includes an AM/PM designator.
     * @param {CalendarNodeType} calNode - The calendar node containing locale-specific data.
     * @returns {string} The formatted time string with colons as separators.
     */
    static _formatTimeWithColons(timeStr, format, calNode) {
        const allDigitsRegex = /^\d+$/;
        // JET-71233: For lenient parsing without separators or fractional seconds, insert a separator
        // every 2 digits from the end to produce the correct grouping of hour/minute/seconds.
        // E.g. '100' → '1:00', '1000' → '10:00', '10000' → '1:00:00'. Otherwise, the RegExp is greedy
        // and tries to consume 2 digits for hour, 2 digits for minutes, and 2 digits for seconds.
        // First pull out the am/pm from the timepart so that we can format the alldigits piece.
        let ampm = '';
        let timeStrOutput = timeStr;
        if (format.includes('a')) {
            const idx = this._matchPMSymbol(calNode, timeStr);
            if (idx) {
                ampm = PM_DESIGNATOR;
            }
            else {
            ...
            const localizedAM = calNode.dayPeriods.format.wide.am.normalize('NFKC');
            const localizedPM = calNode.dayPeriods.format.wide.pm.normalize('NFKC');
            // Remove am/pm (it will be a localized string) from timeToParse
            // In some locales you have a. m. but it's reasonable that they type a.m., so we deal
            // with that too.
            timeStr = timeStr
                .replace(localizedPM, '')
                .replace(localizedAM, '')
                .replace(NativeParserImpl._removeSpaces(localizedPM), '')
                .replace(NativeParserImpl._removeSpaces(localizedAM), '')
                .replace(PM_DESIGNATOR, '')
                .replace(AM_DESIGNATOR, '')
                .replace(PM_DESIGNATOR.toLowerCase(), '')
                .replace(AM_DESIGNATOR.toLowerCase(), '')
                .trim();
        }
        // format the alldigits piece with colons, and then add back the ampm string.
        if (allDigitsRegex.test(timeStr) && !format.includes('.s')) {
            const chars = [];
            const len = timeStr.length;
            const groupSize = 2;
            const firstGroup = len % groupSize || groupSize;
            for (let i = 0; i < len;) {
                const size = i === 0 ? firstGroup : groupSize;
                chars.push(timeStr.slice(i, i + size));
                i += size;
            }
            timeStrOutput = chars.join(':');
            if (ampm !== '') {
                timeStrOutput += ' ' + ampm;
            }
        }
        return timeStrOutput;
    }
    /*
...
const normalizeIsoString = (timeZone, value) => {
    ...
    if (value.startsWith('T')) {
        ...
        // This fixes bug JET-77344 DateTimeFormat time-only format is one hour off during DST if timeZone different than system.
        // It does not affect the parse code.
        const curDate = dateToLocalIso(new Date()).split('T')[0];
        value = curDate + value;
    }
...
export { CalendarUtils as C, NativeDateTimeConstants as N, getFormatParse as a, isLeapYear as b, isTimeOnlyIsoString as c, getISODateOffset as d, NativeParserImpl as e, formatWithYearFormat as f, getDaysInMonth as g, dateTimeUtils as h, isDateOnlyIsoString as i, normalizeIsoString as n };
//# sourceMappingURL=getFormatParse-0b8d78e0.js.map
