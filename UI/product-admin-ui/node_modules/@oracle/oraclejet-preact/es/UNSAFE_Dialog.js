...
export { D as Dialog } from './Dialog-5f3bd525.js';
 ...
 */
const useFlexibleComponent = ({ isDisabled = false, isDraggable = false, isResizable = false, onDragStart: onDragStartHandler, onDragMove: onDragMoveHandler, onDragEnd: onDragEndHandler, onResizeStart: onResizeStartHandler, onResize: onResizeHandler, onResizeEnd: onResizeEndHandler }) => {
    const [coords, _setCoords] = useState();
    const [dimensions, setDimensions] = useState();
    const coordsRef = useRef(null);
    const origPosRef = useRef(null);
    const origSizeRef = useRef(null);
    ...
    const onResizeStart = (rect) => {
        setCoords({ x: rect.x, y: rect.y });
        origPosRef.current = { x: rect.x, y: rect.y };
        setDimensions({ width: (rect.width + 'px'), height: (rect.height + 'px') });
