...
import { L as Link } from './Link-49801aeb.js';
import { T as TruncatingLink } from './TruncatingLink-cfd6e8a3.js';
...
import './useMiddleEllipsis-b073ca6b.js';
 ...
 */
const LabelledLink = ({ 'aria-describedby': ariaDescribedBy, children, columnSpan, href, label, labelEdge: propLabelEdge, labelStartWidth: propLabelStartWidth, lineClamp, onClick: propOnClick, target, truncation = 'end', textAlign: propTextAlign, userAssistanceDensity: propUserAssistanceDensity }) => {
    ...
    const { baseId, formFieldContext: textFieldFormFieldContext, labelProps, inputProps, userAssistanceProps } = useTextField({
        ariaDescribedBy,
        isReadonly: true,
        labelEdge,
        value: linkText
    ...
    const linkWasActivatedRef = useRef(false);
    ...
    const onFocusWithin = useCallback(() => {
        ...
        const isTruncated = labelEdge === 'inside' && !recentPointerRef.current ? detectTruncation(labelRef) : false;
        setShowTruncatedTooltip(isTruncated);
    }, [labelEdge, labelRef]);
    const { focusProps: { onFocusIn: onFocus, onFocusOut: onBlur } } = useFocusWithin({
        isDisabled: false,
        onBlurWithin,
        onFocusWithin
    });
    ...
    //prevent link text or href truncation for mixed readonly
    if (!isFormReadonly && isFormLayout) {
        truncation = 'none';
    }
    return (jsx(FormFieldContext.Provider, { value: formFieldContext, children: jsx(ReadonlyTextField, { columnSpan: columnSpan, role: "presentation", inlineUserAssistance: inlineUserAssistance, ...fieldLabelProps, mainContent: jsxs("div", { onBlur: onBlur, onFocus: onFocus, onPointerDown: onPointerDown, class: classes, ...containerAria, children: [hasHiddenLabel && jsx(HiddenLabel, { ...hiddenLabelAria, children: label }), jsx(View, { width: "100%", children: lineClamp ? (jsx(TruncatingLink, { lineClamp: lineClamp, href: href, target: target, onClick: onClick, ...linkAria, children: linkText })) : truncation !== 'none' ? (jsx(TruncatingLink, { truncation: truncation, href: href, target: target, onClick: onClick, ...linkAria, children: linkText })) : (jsx(Link, { href: href, target: target, onClick: onClick, ...linkAria, children: linkText })) })] }) }) }));
};
