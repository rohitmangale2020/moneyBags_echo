...
import { P as ProgressButtonLabelLayout } from './ProgressButtonLabelLayout-765b3a67.js';
...
import { u as useProgressLoadingState } from './useProgressLoadingState-7807cc64.js';
...
import './useProgressButtonLabelLayoutTheme-4116ba69.js';
...
import './ProgressCircle-d1f1a500.js';
import './meterUtils-19c07f4c.js';
import './utils-f4b2e9db.js';
import './circleUtils-90dae0a2.js';
...
import './CircularMeter-f2c0ae34.js';
import './withThemeProps-dbd4fedf.js';
 ...
 */
const ProgressButton = forwardRef(({ variant = 'outlined', isLoading = false, isDisabled = false, size = 'md', display = 'all', type: buttonType = 'button', startIcon, edge, testId, onAction, label = '', tooltip, 'aria-describedby': ariaDescribedBy, width, ...props }, ref) => {
    const { globalEventProps } = processGlobals(props);
    const isAdjustedLoading = isDisabled ? false : isLoading;
    const progressStatus = bundle.accButtonProgressStatus({
        label: label
    });
    const progressComplete = bundle.accButtonProgressComplete({
        label: label
    });
    ...
    const { loaded } = useProgressLoadingState(isLoading);
    const labelOnly = !startIcon || display === 'label';
    const elementDetails = {
        type: 'button',
        buttonType: buttonType
    };
    const content = (jsx(ProgressButtonLabelLayout, { display: display, isLoadingContrast: variant === 'callToAction', startIcon: startIcon, size: size, isLoading: isAdjustedLoading, children: label }));
    /* For a11y, we use ignore aria-label during loading time */
    const { 'aria-describedby': adb, ...ttProps } = tooltipProps;
    return (jsxs(Fragment, { children: [jsx(BaseButton, { ...mergeProps(ttProps, globalEventProps), ref: ref, elementDetails: elementDetails, isDisabled: isDisabled, isReadOnly: isAdjustedLoading, size: size, width: width, edge: edge, variant: variant, styling: labelOnly
                    ? isAdjustedLoading
                        ? ['default', 'hiddenLabel', 'noHoverActive']
                        : ['default']
                    : isAdjustedLoading
                        ? ['noHoverActive', 'min']
                        : ['min'], onAction: onAction, "aria-label": label, testId: testId, children: content }), tooltipContent, jsx(LiveRegion, { type: "assertive", children: loaded ? progressComplete : isLoading ? progressStatus : '' })] }));
});
