/* @oracle/oraclejet-preact: undefined */
import { jsxs, Fragment, jsx } from 'preact/jsx-runtime';
import { useRef, useCallback } from 'preact/hooks';
import { u as useTranslationBundle } from './useTranslationBundle-4822502f.js';
import { S as SvgHelp } from './Help-93fa3375.js';
import { P as Popup } from './Popup-d782a727.js';
import { H as HiddenAccessible } from './HiddenAccessible-61197e7c.js';
import { iconUserAssistancePopup, iconUserAssistanceIcon } from './UNSAFE_UserAssistance/themes/UserAssistanceStyles.css.js';
import { u as useComponentTheme } from './useComponentTheme-5f800588.js';
import { c as classNames } from './classNames-4e12b00d.js';
import { PopupRedwoodTheme } from './UNSAFE_Popup/themes/redwood/PopupTheme.js';
import { u as useToggle } from './useToggle-8b7fcefe.js';
import { I as InlineHelp } from './InlineHelp-774fd01d.js';

/**
 * Created to match icon user assistance spec for use with radio option, pending design review
 * of inline UA assistance.
 */
const IconUserAssistance = ({ assistiveText, helpSourceLink, helpSourceText, isTabbable = true, id, variant = 'checkbox' }) => {
    const { bool: isOpen, setFalse: setIsOpenFalse, toggle } = useToggle(false);
    const translations = useTranslationBundle('@oracle/oraclejet-preact');
    const userAssistance_learnMoreStr = translations.userAssistance_learnMore();
    const sourceText = helpSourceLink && (helpSourceText || userAssistance_learnMoreStr);
    const helpForRadioString = translations.radio_helpForRadio();
    const helpForCheckboxString = translations.checkbox_helpForCheckbox();
    const helpStringId = `${id}-help-string`;
    const anchorRef = useRef(null);
    const handleOutsideClick = useCallback((event) => {
        const target = event?.target;
        // We need to filter out anchor click events, because anchor is responsible for the toggling
        if ((target instanceof Element || target instanceof Document) &&
            !anchorRef.current?.contains(target) &&
            isOpen) {
            setIsOpenFalse();
        }
    }, [isOpen, setIsOpenFalse]);
    const handleLinkEvent = useCallback((e) => {
        // if it's a key event that is not "enter", return early
        const { key } = e;
        if (key && key !== 'Enter') {
            return;
        }
        // We want to close the pop up if a link is triggered with mouse or keyboard
        const { tagName } = e.target;
        if (tagName?.toLowerCase() === 'a') {
            setIsOpenFalse();
            // we want to refocus to the help icon after the browser has launched the link
            // prematurely doing so prevents keyboard events from firing
            setTimeout(() => anchorRef.current?.focus());
        }
    }, [setIsOpenFalse]);
    const handleOnKeyDown = useCallback((e) => {
        // allow user to close popup without having to focus it
        if (['Tab', 'Escape'].includes(e.code) && isOpen) {
            setIsOpenFalse();
        }
        // toggle and prevent scroll jump
        if (['Space', 'Enter'].includes(e.code)) {
            e.preventDefault();
            toggle();
        }
    }, [isOpen, setIsOpenFalse, toggle]);
    const onClickHandler = useCallback((e) => {
        e.preventDefault();
        toggle();
    }, [toggle]);
    // We are referencing popup vars, so we need to bring in the baseTheme for popup
    const { baseTheme: popupBaseTheme } = useComponentTheme(PopupRedwoodTheme);
    const iconUserAssistanceClasses = classNames([popupBaseTheme, iconUserAssistancePopup]);
    return helpSourceLink || assistiveText ? (jsxs(Fragment, { children: [jsx("span", { onKeyDown: handleOnKeyDown, onClick: onClickHandler, role: "button", "aria-labelledby": helpStringId, "aria-haspopup": "dialog", ref: anchorRef, tabIndex: isTabbable ? 0 : -1, class: iconUserAssistanceIcon, id: `${id}-help-icon`, children: jsx(SvgHelp, {}) }), jsx(HiddenAccessible, { id: helpStringId, isHidden: true, children: variant === 'checkbox' ? helpForCheckboxString : helpForRadioString }), jsx(Popup, { placement: "top", isOpen: isOpen, anchorRef: anchorRef, onClose: setIsOpenFalse, onClickOutside: handleOutsideClick, shiftOptions: { mainAxis: true, crossAxis: false }, maxWidth: "296px", children: jsx("div", { id: id, tabIndex: 0, class: iconUserAssistanceClasses, onMouseUp: handleLinkEvent, onKeyDown: handleLinkEvent, children: jsx(InlineHelp, { assistiveText: assistiveText, sourceLink: helpSourceLink, sourceText: sourceText }) }) })] })) : null;
};

export { IconUserAssistance as I };
//# sourceMappingURL=IconUserAssistance-30afcebe.js.map
