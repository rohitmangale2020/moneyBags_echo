...
import { g as getTabWidths } from './tabBarUtils-71bd0da1.js';
...
function useDetectHorizontalOverflow({ rootRef, keysArray, isTruncationDisabled = true }) {
    ...
    const isAllItemsTruncatedMax = useRef(false);
    const isAllItemsTruncatedFinal = useRef(false);
    const tabDimensions = useRef();
    useLayoutEffect(() => {
            ...
            if (tabMaxWidth === undefined) {
                ...
                if (!isOverflow) {
                    ...
                    if (!isAllItemsTruncatedMax.current) {
                        const tabWidths = getTabWidths(rootRef.current, trailingContentWidths, isTruncationDisabled, tabBarWidthCurrent, isAllItemsTruncatedMax?.current, keysArray);
                    ...
                    if (arr && tabBarWidthCurrent < arr[lastIndex]) {
                        ...
                        if (closestEntry && closestIndex != null) {
                            ...
                            if (closestIndex > 0 && !isAllItemsTruncatedFinal.current && !isTruncationDisabled) {
                                const tabWidthsAfterOverflow = getTabWidths(rootRef.current, trailingContentWidths, isTruncationDisabled, tabBarWidthCurrent, isAllItemsTruncatedMax?.current);
    ...
    const updateTabBarWidth = useCallback((entry) => {
...
//# sourceMappingURL=useDetectHorizontalOverflow-154005bb.js.map
