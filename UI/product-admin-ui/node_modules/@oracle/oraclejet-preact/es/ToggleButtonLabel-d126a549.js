...
const ToggleButtonInput = ({ isDisabled, isChecked, isSwitch, inputName, inputType, value, inputId, startIcon, endIcon, display, onAction, size, label, onBlur, onFocus, onMouseEnter, onMouseLeave, onTouchEnd, onTouchStart }) => {
    ...
    const content = isLabel ? (jsxs("div", { class: styles.inputLabel, children: [inputElement, label] })) : (jsxs(Fragment, { children: [inputElement, jsx(ButtonLabelLayout, { display: display, startIcon: startIcon, endIcon: endIcon, size: size, children: label }), isIcon ? jsx(HiddenAccessible, { children: label }) : undefined] }));
    // JET-75329: For iOS with VoiceOver, we want to apply the role to
    // the label for proper swiping focus and label announcement.
    const isIos = getClientHints().platform === 'ios';
    const labelRole = isSwitch ? 'switch' : inputType;
    return (jsx("label", { ...(isIos
            ? {
                role: labelRole,
                'aria-checked': isChecked,
                'aria-disabled': isDisabled
            }
            : {}), for: inputId, class: wrapperClasses, onBlur: onBlur, onFocus: onFocus, onMouseEnter: onMouseEnter, onMouseLeave: onMouseLeave, onTouchEnd: onTouchEnd, onTouchStart: onTouchStart, children: content }));
};
...
const ToggleButtonLabel = ({ isDisabled, isSelected, isToggle = true, inputName, inputType, value, startIcon, endIcon, display, onAction, size, label, onBlur, onFocus, onMouseEnter, onMouseLeave, onTouchEnd, onTouchStart }) => {
    ...
    if (!isInput) {
        ...
        return labelOnly ? (jsx(Fragment$1, { children: label })) : (jsx(ButtonLabelLayout, { display: display, startIcon: startIcon, endIcon: endIcon, size: size, children: label }));
    }
    else {
        // For input types, must use labels and modify approach for button
        return jsx(Fragment$1, { children: inputElement });
    }
};
ToggleButtonLabel.displayName = 'ToggleButtonLabel';

export { ToggleButtonLabel as T };
//# sourceMappingURL=ToggleButtonLabel-d126a549.js.map
