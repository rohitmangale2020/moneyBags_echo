...
import { g as getWeeksInMonth, a as getDaysOfWeekNames, b as getStartOfWeek, c as getDatesInWeek } from './dayUtils-d17b681f.js';
import { a as getFormatParse } from './getFormatParse-0b8d78e0.js';
import { w as warn } from './logger-35b2acf7.js';
import { d as getIsoDateStr, t as toJSDate, e as toJSDateUTC, b as checkNoCalendarExtension, f as formatFullCalendarDate, i as isDateOutOfRange } from './calendarDateUtils-a6baf09c.js';
import { useMemo, useRef } from 'preact/hooks';
import { S as StyledDatePickerButton } from './StyledDatePickerButton-a7782821.js';
...
import { weekNumberCellStyles, weekNumberStyles } from './PRIVATE_MonthView/themes/WeekNumberCellStyle.css.js';
 ...
 */
const useDayCell = ({ date, dayFormatter = defaultDayFormatter, dateFormatter, isAdjacentMonth, isDisabled, isFocusable, isFocused, isHidden, isReadonly, isSelected, isToday, isForcedFocusRing, onAction, onNavigation }) => {
...
const calculateWeekNumber = (date) => {
    ...
    if (dayNumber !== 0 && dayNumber !== 1 && dayNumber !== 6) {
        ...
        warn(`calculateWeekNumber's date property's getUTCDay() is assumed to be 0 (Sunday), 1 (Monday) or 6 (Saturday)
...
const WeekNumberCell = ({ startDateOfWeek }) => {
    ...
    const translations = useTranslationBundle('@oracle/oraclejet-preact');
    return (jsx("div", { "aria-label": translations.datePicker_weekNum({ NUM: String(weekNumber) }), class: weekNumberCellStyles, role: "rowheader", children: jsx("span", { class: weekNumberStyles, children: weekNumber }) }));
};
 ...
 */
const MonthView = ({ dayFormatter, daysOutsideMonth = 'hidden', isReadonly = false, max, min, onAction, onNavigation, selectedDate, focusableDate, isFocused = false, testId, todaysDate, weekDisplay = 'none' }) => {
    ...
    /**
     * JET-77041 acc: safari: oj-c-datepicker focus ring missing after certain interactions
     * When a day cell receives focus via arrow key navigation after a mouse click, Safari
     * ignores `:focus-visible` even though on other browsers it matches.
     * If you tab first, then use the arrow key, Safari does not ignore `:focus-visible`.
     *
     * For Safari only, during keyboard navigation, add a style class that forces the ring on the day cell.
     */
    const dayGridRootRef = useRef(null);
    // This hook is used to track if the user is using the keyboard in the MonthView.
    // We map focusRingVisible to isFocusRingShown. This is true when the user uses the keyboard,
    // and set to false when the user uses the mouse.
    const [isFocusRingShown, focusRingProps] = useCollectionFocusRing(dayGridRootRef, ['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', 'Home', 'End', 'PageUp', 'PageDown'], true // isVisibleFocusLostOnClick, click should hide ring
    );
    const isSafari = getClientHints().browser === 'safari';
    const isForcedFocusRing = isSafari && isFocusRingShown;
    const renderMonthViewHeaderRow = () => {
        const weekDays = getDaysOfWeekNames(locale, 'short');
        return (jsx("div", { class: styles.sticky, children: jsxs(Grid, { role: "row", "aria-hidden": "true", gridTemplateColumns: gridTemplateColumns, gap: monthViewScaleVars.columnGap, children: [isWeekNumberDisplayed && jsx("div", { role: "presentation" }), weekDays.map((day) => (jsx("div", { role: "columnheader", class: styles.dayOfTheWeek, children: jsx("span", { children: day }) })))] }) }));
    };
    const renderMonthView = () => {
        ...
        for (let weekIndex = 0; weekIndex < weeksInMonth; weekIndex++) {
            ...
            const weekCells = weekDates.map((dayCellDate) => {
                ...
                return (jsx(DayCell, { date: dayCellDate, dateFormatter: dateFormatter, dayFormatter: dayFormatter, isAdjacentMonth: isAdjacentMonth, isDisabled: isDisabled, isFocusable: isFocusable, isFocused: isFocusable && isFocused, isForcedFocusRing: isForcedFocusRing, isHidden: isHidden, isReadonly: isReadonly, isSelected: isSelected, isToday: isToday, onAction: onAction, onNavigation: onNavigation }, `${dayCellDate.year}-${dayCellDate.month}-${dayCellDate.day}`));
            });
            calendarGrid.push(jsxs(Grid, { role: "row", gridTemplateColumns: gridTemplateColumns, gap: monthViewScaleVars.columnGap, children: [isWeekNumberDisplayed && jsx(WeekNumberCell, { startDateOfWeek: weekDates[0] }), weekCells] }, weekIndex));
        }
        return calendarGrid;
    };
    const monthViewAriaLabel = formatFullCalendarDate(locale, focusableDate, 'month');
    return (jsx("div", { class: baseTheme, ref: dayGridRootRef, ...focusRingProps, ...testIdProps, children: jsxs(Grid, { gap: monthViewScaleVars.rowGap, "aria-label": monthViewAriaLabel, role: "grid", children: [renderMonthViewHeaderRow(), renderMonthView()] }) }));
};
...
//# sourceMappingURL=MonthView-65906260.js.map
