...
import { useRef, useLayoutEffect, useCallback, useState, useImperativeHandle, useMemo } from 'preact/hooks';
import { a as getRowKey, b as getElementBySelector, g as getCellType, c as getRowIndexForRowElement$1, d as getLastRenderedRowKey$1, e as getFirstRenderedRowKey$1, i as isHTMLScroller, f as getVerticalOverflowDiff, h as getHorizontalOverflowDiff, j as getScrollbarHeight, k as isLeftHTMLScrollbar, l as getScrollbarWidth } from './ScrollUtils-8a50c01e.js';
...
import { u as useAddBusyState } from './useAddBusyState-7a6af61f.js';
import { k as keyExtractor, f as findElementByKey } from './collectionUtils-609f73d6.js';
import { scrollerStyles } from './PRIVATE_Table/themes/Table.css.js';
import { textAlignStyles, verticalAlignStyles } from './PRIVATE_TableCommon/themes/Common.css.js';
import { tableCellStylesHC, tableCellStyles } from './PRIVATE_Table/themes/TableCellStyles.css.js';
...
const getRowIndexForRowElement = (rootElement, tableId, rowElement) => {
    return getRowIndexForRowElement$1(rootElement, tableId, rowElement, TABLE_ROW_SELECTOR);
};
/**
...
function logicalCellExtractor(element, tableId, columnsArray, cellSelector, onlyAncestors) {
    ...
    if (startingElement != null) {
        ...
        if (cellElement != null) {
            ...
            if (cellType === 'data') {
                return {
                    rowKey: keyExtractor(cellElement, `[${TABLE_ROW_SELECTOR}='${tableId}']`),
...
const getFirstRenderedRowKey = (rootElement, tableId) => {
    return getFirstRenderedRowKey$1(rootElement, tableId, TABLE_ROW_SELECTOR);
};
/**
 * Returns the key of the last focusable row in the Table
 */
const getLastRenderedRowKey = (rootElement, tableId) => {
    return getLastRenderedRowKey$1(rootElement, tableId, TABLE_ROW_SELECTOR);
};
/**
...
const _getScrollbarHeight = (scrollerElement, sizingInfo) => {
    return sizingInfo.isInitialRender
        ? getScrollbarHeight(scrollerElement)
...
const _getScrollbarWidth = (scrollerElement, viewportInfo, sizingInfo, overflowInfo) => {
    if (viewportInfo.isExternalScroller || overflowInfo.isVertical) {
        return sizingInfo.isInitialRender
            ? getScrollbarWidth(scrollerElement)
...
const getScrollerStart = (scrollerElement, viewportInfo, sizingInfo, overflowInfo, stickyOffset, isRtl) => {
    ...
    if (isRtl) {
        if (isHTMLScroller(scrollerElement)) {
            const scrollbarOffset = isLeftHTMLScrollbar(scrollerElement)
...
function _scrollColumnIntoView(columnElement, scrollerElement, viewportInfo, columnsArray, stickyColumnInfo, sizingInfo, overflowInfo, isRtl, appliedColumnWidths, location) {
    ...
    if (columnIndex != null && appliedColumnWidths != null) {
        ...
        const horizontalOverflowDiff = getHorizontalOverflowDiff(columnElement, getScrollerStart(scrollerElement, viewportInfo, sizingInfo, overflowInfo, startOffset, isRtl), getScrollerEnd(scrollerElement, viewportInfo, sizingInfo, overflowInfo, endOffset, isRtl), isRtl);
...
const _scrollRowIntoView = (rootElement, scrollerElement, viewportInfo, sizingInfo, tableId, rowElement, location) => {
    const verticalOverflowDiff = getVerticalOverflowDiff(rowElement, getScrollerTop(rootElement, scrollerElement, viewportInfo.viewportConfig, tableId), getScrollerBottom(rootElement, scrollerElement, viewportInfo.viewportConfig, sizingInfo, tableId));
...
function applyScrollDetail(rootElement, scrollerElement, viewportInfo, tableId, columnsArray, stickyColumnInfo, sizingInfo, overflowInfo, scrollDetail, isRtl, appliedColumnWidths) {
    if (!sizingInfo.isInitialRender) {
        if (scrollDetail.cell.type === 'data') {
            const rowElement = findElementByKey(rootElement, scrollDetail.cell.rowKey, `[${TABLE_ROW_SELECTOR}='${tableId}']`);
...
function hideOutOfViewTooltip(rootElement, scrollerElement, viewportInfo, tableId, stickyColumnInfo, sizingInfo, overflowInfo, fullColumnWidths, isRtl, tooltipAnchorRef, hideTooltip) {
    if (tooltipAnchorRef?.current != null && hideTooltip != null) {
        const verticalOverflowDiff = getVerticalOverflowDiff(tooltipAnchorRef.current, getScrollerTop(rootElement, scrollerElement, viewportInfo.viewportConfig, tableId), getScrollerBottom(rootElement, scrollerElement, viewportInfo.viewportConfig, sizingInfo, tableId));
        const topOverflowDiff = verticalOverflowDiff.top;
        const hasTopOverflow = topOverflowDiff >= 0;
        const bottomOverflowDiff = verticalOverflowDiff.bottom;
        const hasBottomOverflow = bottomOverflowDiff >= 0;
        ...
        const horizontalOverflowDiff = getHorizontalOverflowDiff(tooltipAnchorRef.current, getScrollerStart(scrollerElement, viewportInfo, sizingInfo, overflowInfo, startOffset, isRtl), getScrollerEnd(scrollerElement, viewportInfo, sizingInfo, overflowInfo, endOffset, isRtl), isRtl);
        const leftOverflowDiff = horizontalOverflowDiff.left;
        const hasLeftOverflow = leftOverflowDiff >= 0;
        const rightOverflowDiff = horizontalOverflowDiff.right;
        const hasRightOverflow = rightOverflowDiff >= 0;
...
function useSizingManager({ rootRef, innerTableRef, tableId, tableData, viewportConfig, externalScrollerAccessRef, layout, columns, columnOrder, columnWidths, isRtl, isHighContrast }) {
    ...
    const { onBusyStart: onResizeScrollerBusyStart, onBusyEnd: onResizeScrollerBusyEnd } = useAddBusyState('handling scroller resize');
    const { onBusyStart: onResizeTableBusyStart, onBusyEnd: onResizeTableBusyEnd } = useAddBusyState('handling table resize');
    // state to force a rerender following the initial render where scrollbars are forced
    const [_isScrollbarSizeKnown, setIsScrollbarSizeKnown] = useState(false);
    // column width state information
    const sampledColumnNetWidthsRef = useRef();
    const [appliedColumnWidths, setAppliedColumnWidths] = useState();
    const [innerTableHeight, setInnerTableHeight] = useState();
    const [hasVerticalUnderflow, setHasVerticalUnderflow] = useState(false);
    // general table sizing state information
    const sizingInfoRef = useRef({
        isInitialRender: true,
        isScrollbarSizeKnown: false
    });
    const pendingResizeEntriesRef = useRef({});
    // scrollbar overflow state
    const overflowInfoRef = useRef({
        isHorizontal: false,
        isVertical: false
    });
    ...
    const stickyColumnInfo = useMemo(() => {
    ...
    const columnBorderEdges = useMemo(() => {
    ...
    const _updateStickyEdgesFromResize = useCallback((rootElement, scrollerElement) => {
        if (appliedColumnWidths != null) {
    ...
    const handleResizeCallbackRef = useRef(handleResizeCallback);
    ...
    const staticHandleScrollerResize = useCallback((entry) => {
    ...
    const staticHandleInnerTableResize = useCallback((entry) => {
        // only queue up an animation frame if there isn't one already pending
        if (pendingResizeEntriesRef.current.scroller == null ||
            pendingResizeEntriesRef.current.innerTable == null) {
    ...
    useResizeObserver(viewportInfo.viewportConfig.scrollerRef, staticHandleScrollerResize);
...
function TableSkeletonRow({ tableId, colspan, hasTabIndex, isShowFocusRing, isEmbedded, isPendingLayout = false, isLoadMore = false, isHighContrast }) {
    ...
    return (jsx("tr", { class: LOADMORE_STYLE_CLASS, role: 'row', children: jsx("td", { class: cellClasses, colSpan: colspan, role: 'gridcell', tabIndex: hasTabIndex ? 0 : -1, "data-oj-cell-type": isPendingLayout ? 'pending' : isLoadMore ? 'loadMore' : 'loading', ...(isPendingLayout
                ? { 'data-oj-table-pending': tableId }
                : isLoadMore
                    ? { 'data-oj-table-loadmore': tableId }
                    : { 'data-oj-table-loading': tableId }), "data-oj-table-focusable": tableId, children: jsxs("div", { class: innerCellClasses, children: [accCellContent, [...Array(skeletonsCount)].map((_element, index) => (jsx("div", { class: index === 0 ? firstRowClasses : rowClasses, children: jsx(TableSkeleton, { height: "4x", isHighContrast: isHighContrast }) })))] }) }) }));
}
...
function TableColGroup({ tableId, isRendered, columnWidthsArray }) {
    ...
    return isRendered ? (jsx("colgroup", { children: columnWidthsArray.map((columnWidth) => {
            return jsx("col", { style: getColStyle(columnWidth), "data-oj-table-col": tableId });
        }) })) : null;
}

export { getFormTextAlign as A, TableSkeletonRow as B, getHeaderFooterCellClassArray as C, useSizingManager as D, TableColGroup as E, TABLE_NODATA_SELECTOR as F, TABLE_FOCUSABLE_SELECTOR as T, applyScrollDetail as a, getBoundaryColumnKey as b, getColumnKey as c, getLastRenderedRowKey as d, TABLE_ROW_SELECTOR as e, hasLoadMoreCell as f, getElementFromCell as g, hasFooterCells as h, isAutoFocusCell as i, getFirstRenderedRowKey as j, hasNoDataCell as k, logicalCellExtractor as l, getVerticalScrollPosition as m, getRowIndexForRowKey as n, getScrollerBottom as o, getRowKeyForRowIndex as p, getColumnIndexForHeaderFooterElement as q, resetVerticalScrollPosition as r, getRowIndexForRowElement as s, getHeaderRowElement as t, TABLE_HEADER_SELECTOR as u, useScrollHandling as v, getHorizontalScrollPosition as w, getTableBodyElement as x, getNoDataCellElement as y, getDataCellClassArray as z };
//# sourceMappingURL=TableColGroup-7c7b852c.js.map
