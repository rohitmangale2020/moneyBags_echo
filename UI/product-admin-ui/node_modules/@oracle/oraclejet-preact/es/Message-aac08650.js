...
import { p as playSound, s as severityBasedStyleClass, a as MessageSummary, M as MessageStartIcon } from './MessagesManager-98126752.js';
...
function Message({ detailRenderer, iconRenderer, item, onClose, messageRef = () => { }, variant = 'pageBanner' }) {
     ...
     */
    const handleClose = useCallback((actionDetail) => onClose?.(item, actionDetail), [item, onClose]);
    /**
    ...
    const timeStamp = isValidValueForProp(timestamp, 'timestamp') && variant !== 'toast' && (jsx(MessageTimestamp, { variant: baseVariant, value: timestamp }));
    const messageDetail = (jsx(MessageDetail, { variant: baseVariant, item: item, renderer: detailRenderer }));
    const messageSummary = (jsx(MessageSummary, { variant: baseVariant, text: summary, role: "heading", "aria-level": 2 }));
    ...
    // padding.
    return (jsx("div", { ref: messageRef, class: rootClasses, role: "alert", "aria-atomic": "true", "data-oj-key": `${typeof item.key}-${item.key}`, tabIndex: 0, onKeyUp: handleCloseOnEsc, onFocusIn: pauseTimer, onFocusOut: resumeTimer, children: jsxs("div", { class: contentClasses, children: [iconRenderer ? (jsx(MessageCustomStartIcon, { item: item, renderer: iconRenderer })) : isSeverityIconNeeded(severity) ? (jsx(MessageStartIcon, { severity: severity, variant: baseVariant })) : null, jsx(Flex, { direction: "column", flex: "1", gap: xUnits(2), children: jsxs(Fragment$1, { children: [jsxs(MessageHeader, { variant: baseVariant, children: [summary ? messageSummary : messageDetail, timeStamp] }), summary && messageDetail] }) }), closeAffordance === 'on' && (jsx(MessageCloseButton, { variant: baseVariant, onAction: handleClose }))] }) }));
}

export { Message as M, MessageCloseButton as a, MessageDetail as b, MessageTimestamp as c };
//# sourceMappingURL=Message-aac08650.js.map
