...
import { useRef, useCallback, useEffect, useMemo, useContext, useReducer, useImperativeHandle } from 'preact/hooks';
import { L as Label } from './Label-a6c32235.js';
import { a as isSelectAll, b as isBackspaceOrDelete } from './keyboardUtils-986fe923.js';
import { R as ReadonlyTextField } from './StyledTextField-64f0929e.js';
...
import { b as isLeapYear } from './getFormatParse-0b8d78e0.js';
import { n as getDaysInMonth, r as getMonthsInYear, M as MIN_ISO_YEAR, s as MAX_ISO_YEAR, u as MIN_ISO_MONTH, v as MIN_ISO_DAY, f as formatFullCalendarDate, w as getDatePlaceholderMasks, x as formatShortCalendarDate } from './calendarDateUtils-a6baf09c.js';
import { i as isNumberOnlyString, s as stringToNumber, n as numberToString, w as withinRange, a as getTodayAsCalendarDate, g as getMonthName, t as translateCustomMasks, c as calendarDatesAreDifferent } from './maskUtils-b16a22c0.js';
...
const updateSegment = (segmentType, segmentState, userInput, hasLeadingZero, maxValue, minValue, existingText) => {
    ...
    const newVal = inRange ? requestedVal : stringToNumber(userInput);
...
const useDateFieldHandlers = ({ hasSelectedSegment, value, direction, dispatch, groupRef, isCompleteDate, isDateSelected, isDisabled, isInputFocused, orderedSegments, state, onCommit, onInput }) => {
    ...
    const onKeyDown = useCallback((event) => {
        ...
        else if ((isBackspaceOrDelete(event) || event.key === 'Backspace') && isDateSelected) {
...
const DateField = ({ 'aria-describedby': describedBy, 'aria-invalid': ariaInvalid, 'aria-label': ariaLabel, 'aria-labelledby': ariaLabelledBy, baseId, dispatch, fieldRef, granularity = 'day', hasInsideLabel, isRequired, masks: dateMasks, onBlur, onCommit, onFocus, onInput, state, textAlign, value, variant = 'default' }) => {
    ...
    const formattedDateStr = !isCompleteDate
        ? ''
        : formatFullCalendarDate(locale, value, granularity);
    const formattedDateAriaId = `${baseId}-date`;
    const formattedDateAria = isCompleteDate ? (jsx(HiddenAccessible, { id: formattedDateAriaId, children: formattedDateStr })) : undefined;
    const ariaDescribedBy = isCompleteDate ? l([describedBy, formattedDateAriaId]) : describedBy;
    ...
    return (jsxs("div", { "aria-describedby": ariaDescribedBy, "aria-label": ariaLabel, "aria-labelledby": ariaLabelledBy, class: compClasses, onFocusIn: onFocus, onFocusOut: onBlur, ref: groupRef, role: "group", tabIndex: tabIndex, ...dateFieldHandlers, children: [jsx(Flex, { direction: "row", wrap: "nowrap", justify: textAlign, height: "100%", align: "center", children: dateMasks.map(({ type, value: placeholder }) => {
                    if (type === 'literal') {
                        ...
                        if (segmentProps) {
                            return (jsx(DateSegment, { inputRef: type === firstSegmentType ? fieldRef : undefined, isDisabled: isDisabled, isFieldFocused: isFocused, isInvalid: !!ariaInvalid, isHidden: isHidden, isHighlighted: isDateSelected, isRequired: isRequired, orderedSegments: orderedSegments, placeholder: placeholder, ...segmentHandlers, ...segmentProps }));
                        }
                        return null;
                    }
...
const InputDateMask = forwardRef(({ 'aria-describedby': ariaDescribedBy, assistiveText, columnSpan, endContent, granularity = 'day', helpSourceLink, helpSourceText, isDisabled: propIsDisabled, isReadonly: propIsReadonly, isRequired, isRequiredShown, label, labelEdge: propLabelEdge, labelStartWidth: propLabelStartWidth, mainFieldRef, messages, masks, testId, textAlign: propTextAlign, userAssistanceDensity: propUserAssistanceDensity, value, variant, onCommit, onInput }, ref) => {
    ...
    const lastCommittedDateRef = useRef(value);
    ...
    const lastDateRef = useRef(value);
    const onInputIfDifferent = useCallback((detail) => {
        if (calendarDatesAreDifferent(detail.value, lastDateRef.current)) {
    ...
    const onCommitIfDifferent = useCallback((detail) => {
        if (calendarDatesAreDifferent(detail.value, lastCommittedDateRef.current)) {
    ...
    if (calendarDatesAreDifferent(lastDateRef.current, value)) {
    ...
    // complete.
    const onDateFieldFocusOut = useCallback((event) => {
    ...
    const { onBlur: handleDateFieldBlur } = useMemo(() => {
        return mergeProps({ onBlur: onDateFieldFocusOut }, { onBlur: inputPropsOnBlur });
    ...
    const mainContent = (jsx(DateField, { "aria-label": ariaLabel, "aria-labelledby": ariaLabelledBy, baseId: baseId, dispatch: dispatch, fieldRef: enabledElementRef, granularity: granularity, hasInsideLabel: labelComp !== undefined && labelEdge === 'inside', isRequired: isRequired, masks: dateMasks, onCommit: onCommitIfDifferent, onBlur: handleDateFieldBlur, onInput: onInputIfDifferent, state: state, textAlign: textAlign, value: value, ...dateFieldProps }));
...
//# sourceMappingURL=InputDateMask-80707fa6.js.map
