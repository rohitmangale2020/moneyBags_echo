...
import { useMemo, useCallback, useContext, useRef, useState, useEffect, useImperativeHandle } from 'preact/hooks';
import { S as Sheet } from './Sheet-b5910642.js';
import { forwardRef, useCallback as useCallback$1 } from 'preact/compat';
import { g as getPrevNext, a as getNVisible, b as getMenuPointerDown, c as getMenuContainerMouseLeave, d as getMenuContainerFocusMethods, e as getCloseSubmenuEvent, f as getKeyboardCloseProps, k as keyElemExtractor, h as getLastVisible, D as Dropdown, i as getIsMobile, j as isMenuChildrenFalsy, l as getMenuItemClickChangeKey, m as getOpenSubmenuEvent } from './Dropdown-63170b62.js';
...
import { menuSkeletonStyles } from './UNSAFE_Menu/themes/MenuSkeletonStyles.css.js';
...
const MenuContainer = ({ children, 'aria-label': ariaLabel, currentFocus = 'menu', display, closeSubmenu, menuId, menuLevel, anchorRef }) => {
    ...
    */
    ...
    return (jsx("div", { ...(menuId && { id: menuId }), ref: menuContainerRef, tabIndex: !currentKey && (menuLevel === 'main' || display === 'sheet') ? 0 : -1, role: "menu", "aria-orientation": "vertical", "aria-label": ariaLabel, class: classNames([menuStyles.menuContainerStyle]), ...mergeProps(currentKeyProps, getKeyboardCloseProps(onClose, menuLevel), getCloseSubmenuEvent(direction, closeSubmenu), getMenuContainerFocusMethods(blurFunction, focusFunction), getMenuContainerMouseLeave(mouseLeaveFunction), getMenuPointerDown()), ...testIdProps, children: jsx(MenuContainerContext.Provider, { value: {
...
function MenuSkeletonContainer({ children }) {
    const { baseTheme } = useComponentTheme(MenuItemRedwoodTheme);
    const containerClasses = classNames([baseTheme, menuSkeletonStyles.container]);
    return (jsx("div", { "oj-menu-skeleton": "true", class: containerClasses, role: "presentation", children: [...Array(4)].map((_element, index) => children?.(index)) }));
}
const MenuSkeleton = () => {
    useEffect(() => {
        setTimeout(() => {
    ...
    return (jsxs(Fragment, { children: [isVisible && jsx(LiveRegion, { children: translations.menu_stateLoading() }), jsx(MenuSkeletonContainer, { children: () => {
                    return (jsx(Flex, { height: "11x", align: "center", children: isVisible && jsx(Skeleton, { height: "4x" }) }));
                } })] }));
};
/**
...
const DropdownMenu = forwardRef(({ anchorRef, defaultPlacement, offsetValue, closeSubmenu, children, 'aria-label': ariaLabel, currentFocus, menuId, menuLevel, flipOptions, isOpen, isMenuLoading = false, minWidth, onTransitionEnd }, ref) => {
    const { onClose } = useMenuContext();
    const outsideMousedownHandler = useCallback$1((e) => {
        if (menuLevel === 'sub')
            return;
        ...
        if (e.reason === 'outsideMousedown') {
            ...
            if (!menuItemKey) {
                const isAnchorClick = anchorRef.current?.contains?.(e.target) ||
                    anchorRef.current?.[UNSAFE_DOM_ACCESS]?.contains(e.target);
    ...
    */
    return (jsx(Dropdown, { minWidth: minWidth ? Math.max(DEFAULT_MENU_MIN_WIDTH, minWidth) : DEFAULT_MENU_MIN_WIDTH, onClose: outsideMousedownHandler, isOpen: isOpen, placement: defaultPlacement, anchorRef: anchorRef, flipOptions: flipOptions, offset: offsetValue, shiftOptions: {
            mainAxis: true,
            crossAxis: true
        }, sizeOptions: { isMaxWidthAdjusted: true, isMaxHeightAdjusted: true, maxWidthCeiling: 460 }, onTransitionEnd: onTransitionEnd, children: jsx("div", { class: classNames([dropdownMenuStyles.base, menuStyles.dropdownMenuContainerStyle]), ref: ref, children: isMenuLoading ? (jsx(MenuSkeleton, {})) : (jsx(MenuContainer, { menuLevel: menuLevel, closeSubmenu: closeSubmenu, "aria-label": ariaLabel, currentFocus: currentFocus, menuId: menuId, display: "dropdown", anchorRef: anchorRef, children: children })) }) }));
});
...
const Menu = ({ onClose, isOpen, testId, initialFocus = 'menu', placement, offsetValue, minWidthBehavior, anchorRef, flipOptions, ...menuProps }) => {
    const sheetDismissHandler = useCallback(() => {
        onClose?.({ reason: 'dismissed' });
    }, [onClose]);
    const isMenuLoading = isMenuChildrenFalsy(menuProps.children);
    ...
    useEffect(() => {
        if (isOpen && minWidthBehavior === 'basedOnAnchorWidth') {
            const obtainedAnchorWidth = anchorRef.current?.offsetWidth ||
                anchorRef.current?.[UNSAFE_DOM_ACCESS]?.offsetWidth;
    ...
    return (jsx(MenuContext.Provider, { value: { onClose, isMobile, testId }, children: jsx(Fragment, { children: isMobile ? (jsx(Fragment, { children: jsx(Sheet, { isOpen: isOpen, onClose: sheetDismissHandler, children: jsx("div", { class: classNames([menuStyles.sheetMenuContainerStyle]), children: isMenuLoading ? (jsx(MenuSkeleton, {})) : (jsx(MenuContainer, { ...menuProps, anchorRef: anchorRef, currentFocus: menuCurrentFocus, menuLevel: "main", display: "sheet" })) }) }) })) : (jsx(DropdownMenu, { ...menuProps, anchorRef: anchorRef, isMenuLoading: isMenuLoading, isOpen: isOpen, currentFocus: menuCurrentFocus, menuLevel: "main", defaultPlacement: placement, offsetValue: offsetValue, flipOptions: flipOptions || {
                    crossAxis: true,
                    mainAxis: true,
                    fallbackPlacements: [
                        'bottom-start',
...
const BaseMenuItem = forwardRef(({ id, label, onAction, startIcon, endIcon, role, isChecked, isDisabled = false, variant = 'standard', type, hoverInfo, openSubmenu, menuItemLabelWeight = 'normal', isSubmenuOpen, submenuId, submenuRef, itemKey, keyboardShortcut }, ref) => {
    const { onClose } = useMenuContext();
...
export { BaseMenuItem as B, DropdownMenu as D, Menu as M, SelectMenuGroupContext as S, MenuItem as a, useMenuContext as b, MenuContainer as c, useMenuContainerContext as u };
//# sourceMappingURL=SelectMenuGroupContext-6ee1b1e1.js.map
