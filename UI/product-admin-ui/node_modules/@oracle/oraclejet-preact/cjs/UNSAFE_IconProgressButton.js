 ...
 */
const IconProgressButton = compat.forwardRef(({ variant = 'outlined', isLoading = false, isDisabled = false, size = 'md', type: buttonType = 'button', onAction, width, tooltip, testId, ...props }, ref) => {
    ...
    const { loaded } = useProgressLoadingState.useProgressLoadingState(isAdjustedLoading);
    /* For a11y, we use ignore aria-label during loading time */
    const { 'aria-describedby': adb, ...ttProps } = tooltipProps;
    return (jsxRuntime.jsxs(preact.Fragment, { children: [jsxRuntime.jsx(BaseButton.BaseButton, { ...mergeProps.mergeProps(ttProps, globalEventProps), ref: ref, elementDetails: elementDetails, isReadOnly: isAdjustedLoading, isDisabled: isDisabled, size: size, variant: variant, width: width, styling: isAdjustedLoading ? ['min', 'hiddenLabel', 'noHoverActive'] : ['min'], onAction: onAction, "aria-label": tooltip, testId: testId, children: jsxRuntime.jsx(ProgressButtonLabelLayout.ProgressButtonLabelLayout, { display: 'icons', isLoading: isAdjustedLoading, isLoadingContrast: variant === 'callToAction', startIcon: props.children, size: size }) }), tooltipContent, jsxRuntime.jsx(LiveRegion.LiveRegion, { type: "assertive", children: loaded ? progressComplete : isLoading ? progressStatus : '' })] }));
});
IconProgressButton.displayName = 'IconProgressButton';

exports.IconProgressButton = IconProgressButton;
//# sourceMappingURL=UNSAFE_IconProgressButton.js.map
