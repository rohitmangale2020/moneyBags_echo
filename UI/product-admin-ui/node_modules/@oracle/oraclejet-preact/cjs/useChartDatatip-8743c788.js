...
var VisTabularDatatip = require('./VisTabularDatatip-e79da9de.js');
 ...
 */
function getChartTabularData(data, seriesValue, groupValue, translations, valueFormats, isTimeAxis, defaultXFormatter, defaultYFormatter, isSeriesAssociatedToY2, hasXYValues) {
    let itemValue, itemX, itemY, itemWidthWeight, itemY2;
    ...
    const dataLabel = translations.chart_labelDataLabel();
    if (hasXYValues && 'y' in data) {
        ...
        const formattedValue = valueFormats?.value?.format
            ? valueFormats.value.format(value)
            : defaultYFormatter?.(value);
        if (isSeriesAssociatedToY2) {
            itemY2 = valueFormats?.y2?.format ? valueFormats.y2.format(value) : formattedValue;
        }
        else {
    ...
    if (valueFormats?.value?.isDisplayed != false) {
        if (hasXYValues) {
            tableData.push({
                key: valueFormats?.value?.label || valueFormats?.x?.label || xLabel,
                value: itemX
            });
            tableData.push({
                key: valueFormats?.value?.label || valueFormats?.y?.label || yLabel,
                value: itemY
            });
        }
        else {
            if (isSeriesAssociatedToY2) {
                tableData.push({
                    key: valueFormats?.value?.label || valueFormats?.y2?.label || valueLabel,
                    value: itemY2
                });
            }
            else {
    ...
    if (valueFormats?.label?.isDisplayed != false && data.label) {
        tableData.push({
            key: valueFormats?.label?.label || dataLabel,
            value: data.label
        });
    }
    return tableData;
...
function getDefaultDatatipRenderer(options) {
    ...
    return (context) => {
        ...
        return {
            content: (jsxRuntime.jsx(VisTabularDatatip.VisTabularDatatip, { tableData: getChartTabularData(context.data, options.series.name || options.series.id.toString(), timeValue || options.groups, options.translations, options.valueFormats, options.timeAxisType != null, options.defaultXFormatter, isSeriesAssociatedToY2 ? options.defaultY2Formatter : options.defaultYFormatter, isSeriesAssociatedToY2, options.hasXYValues) })),
...
function getAriaLabelGenerator(series, translations, valueFormats, leafGroupsInfo, xAxis, isDataXAxis, getGroupFromPath, xDefaultLabelFormatter, yDefaultLabelFormatter, y2DefaultLabelFormatter) {
    const getItemAriaLabel = (data, seriesIndex, groupIndex, isSelectionEnabled, isSelected, isDrillable) => {
        ...
        return accUtils.generateAriaLabel(translations, accLabel, {
...
const getTextAndBoundsUtil = (target, rootRef, xAxisTitle, yAxisTitle, y2AxisTitle, xTitleDimention, yTitleDimention, y2TitleDimention, xAxisTickLabels, focusedGroupInfo) => {
    if (target != rootRef?.current) {
        ...
        if (text === xAxisTitle) {
            return {
                text,
                bounds: xTitleDimention
            };
        }
        else if (text === yAxisTitle) {
            return {
                text,
                bounds: yTitleDimention
            };
        }
        else if (text === y2AxisTitle) {
            return {
                text,
                bounds: y2TitleDimention
            };
        }
        else if (dataSet?.['ojPrivateGroupIndex'] && dataSet?.['ojPrivateLevelIndex']) {
            return {
                // @ts-ignore
                bounds: xAxisTickLabels.find(({ dataProps }) => dataProps['data-oj-private-level-index'] === dataSet['ojPrivateLevelIndex'] &&
    ...
    if (target === rootRef?.current && focusedGroupInfo?.isFocusVisible) {
        const dimsAndText = getTextAndDimsInfo(xAxisTickLabels, focusedGroupInfo.groupIndex, focusedGroupInfo.levelIndex);
        return {
            bounds: dimsAndText?.dims,
            text: dimsAndText?.text
        };
    }
    return;
};
const getTextAndDimsInfo = (tickLabels, groupIndex, levelIndex) => {
    return tickLabels?.find((tickLabel) => tickLabel?.dataProps?.['data-oj-private-group-index'] == groupIndex &&
        tickLabel?.dataProps?.['data-oj-private-level-index'] == levelIndex &&
        tickLabel?.isTruncated);
};
...
const useChartDatatip = ({ isDataCursorEnabled, dataCursorPosition, isHoriz, datatip, xScale, getDataItem, getRefItem, getDataItemPos, getRefObjectBounds, getRefAxis, hoveredItemInfo, focusedItemInfo, series, valueFormats, leafGroupsInfo, width, height, findNearest, timeAxisType, touchResponse, isDisabled, defaultXFormatter, defaultYFormatter, defaultY2Formatter, hasXYValues, translations, getGroupFromPath }) => {
        ...
        : !isDisabled
            ? focusedItemInfo?.isCurrent
                ...
                : hoveredItemInfo?.isCurrent &&
                    ((hoveredItemInfo.seriesIndex !== undefined && hoveredItemInfo.groupIndex !== undefined) ||
                        hoveredItemInfo?.refObjId !== undefined)
                    ? hoveredItemInfo
                    : undefined
            : undefined;
    const anchor = focusedItemInfo?.isCurrent || isDataCursorEnabled ? 'element' : 'pointer';
    const elementBounds = focusedItemInfo?.isCurrent
        ? focusedItemInfo.refObjId != null
            ? getRefObjectBounds(focusedItemInfo.refObjId)
            : getDataItemPos(focusedItemInfo.seriesIndex, focusedItemInfo.groupIndex)
        : undefined;
    const { direction } = useUser.useUser();
    const isRtl = direction === 'rtl';
    let placement = 'top-start';
    let offset = layoutUtils.calculateOffset(isRtl, width, elementBounds);
    ...
    let dtContent;
    let dtBorderColor;
    let dtDefaultContainer;
    let item;
    if (activeInfo?.refObjId != undefined) {
        item = getRefItem(activeInfo?.refObjId);
        const axis = item ? getRefAxis(activeInfo.refObjId) : undefined;
        const datatipContext = item && axis
            ? item.type === 'line'
                ? { type: 'referenceLine', axis, data: item }
                : { type: 'referenceArea', axis, data: item }
            : undefined;
        if (datatipContext) {
            const customDatatip = datatip?.(datatipContext);
            dtContent = customDatatip?.content;
            dtBorderColor = customDatatip?.borderColor;
            dtDefaultContainer = customDatatip?.defaultContainer;
            if (!dtContent) {
                dtContent = datatipContext.data.accessibleLabel;
            }
        }
    }
    else if (activeInfo?.seriesIndex != null && activeInfo?.groupIndex != null) {
        ...
        if (datatipContext) {
            ...
            dtDefaultContainer = customDatatip?.defaultContainer;
            if (dtContent !== '') {
                const defaultDatatip = getDefaultDatatipRenderer({
                ...
                dtContent = dtContent || defaultDatatip.content;
                dtBorderColor = dtBorderColor || defaultDatatip.borderColor;
                dtDefaultContainer = dtDefaultContainer || defaultDatatip.defaultContainer;
            }
        }
    }
    const colors = colorUtils.getColorRamp();
    const borderColor = dtBorderColor ||
        (activeInfo &&
    ...
    const { datatipContent, datatipProps } = useDatatip.useDatatip({
        ...
        anchor,
        placement,
        defaultContainer: dtDefaultContainer,
        offset,
        borderColor,
        touchResponse,
        isDisabled
...
exports.getAriaLabelGenerator = getAriaLabelGenerator;
exports.getTextAndBoundsUtil = getTextAndBoundsUtil;
exports.getTextAndDimsInfo = getTextAndDimsInfo;
exports.useChartDatatip = useChartDatatip;
//# sourceMappingURL=useChartDatatip-8743c788.js.map
