...
var usePressClick = require('./usePressClick-7a34624a.js');
...
function useBareButton({ onAction, isDisabled, isRepeat, isClick, elementDetails = { type: 'button', buttonType: 'button' }, testId, ...props }) {
    ...
    // pressHandler can be invoked by both usePress and usePressClick
    // usePress is used for elements that do not generate a click for kb interaction (non-buttons)
    // usePressClick is used for elements that generate a click for kb interaction (buttons)
    // So e could be a PointerEvent or KeyboardEvent
    ...
    const { type: elementType = 'button', isFocusable: isElementFocusable = true, isLabel: isElementLabel = false, isSwitch: isElementSwitch = false, buttonType: elementButtonType, ...elementProps } = { ...elementDetails };
    const buttonType = elementDetails.type === 'button' ? elementButtonType ?? 'button' : undefined;
    const typeProps = elementDetails.type === 'button' && elementButtonType === 'button' ? { type: buttonType } : {};
...
//# sourceMappingURL=useBareButton-463210c9.js.map
