...
var HighlightText = require('./HighlightText-93f2fea0.js');
require('./Grid-156807d4.js');

var ListItemText = require('./ListItemText-3e93c76c.js');
...
var List = require('./List-71a2ac8b.js');
var usePress = require('./usePress-886180e4.js');
var CaretDown = require('./CaretDown-a62d0c7e.js');
 ...
 * @ignore
 */
function renderItemText(item, itemText, componentName) {
    ...
    if (typeof itemText === 'string') {
        ...
        // or leaving the property unset for the empty state. Older releases already handled that misconfiguration
        ...
        logger.error(`${componentName}: itemText specifies field that does not exist: ${itemText}`);
...
const noOp = () => { };
function DefaultList({ 'aria-label': ariaLabel, componentName, currentItemVariant, currentKey, data, itemRenderer, itemText, onCurrentKeyChange, onItemAction, onLoadRange = noOp, onSelectionChange, searchText, selectedKeys, selectionMode }) {
    const listItemRenderer = hooks.useCallback((listItemContext) => {
        ...
        return itemRenderer ? (itemRenderer(itemRendererProps)) : (jsxRuntime.jsxs(Flex.Flex, { align: "center", children: [listItemContext.selector && (jsxRuntime.jsx("div", { class: PRIVATE_SelectCommon_themes_dropdownStyles_css.dropdownStyles.selectorWrapper, children: listItemContext.selector() })), jsxRuntime.jsx("div", { class: selectionMode === 'single'
                        ...
                        : PRIVATE_SelectCommon_themes_dropdownStyles_css.dropdownStyles.multiSelectItem, children: jsxRuntime.jsx(ListItemText.ListItemText, { variant: "primary", children: jsxRuntime.jsx(HighlightText.HighlightText, { matchText: searchText, children: renderItemText(itemContext, itemText, componentName) }) }) })] }));
    }, [
        componentName,
        itemRenderer,
        itemText,
    ...
    return (jsxRuntime.jsx(List.List, { "aria-label": ariaLabel, allowTabbableMode: false, currentItemVariant: currentItemVariant, currentItemOverride: currentItemOverride, data: data ?? null, onPersistCurrentItem: onCurrentKeyChange, onItemAction: handleItemAction, onLoadRange: onLoadRange, onSelectionChange: handleSelectionChange, role: "listbox", selectedKeys: keys, selectionMode: selectionMode, children: (context) => listItemRenderer(context) }));
}
...
function EmptyResults() {
    ...
    // the input
    return (jsxRuntime.jsx("div", { class: PRIVATE_SelectCommon_themes_dropdownStyles_css.dropdownStyles.results.extraItem, onMouseDown: preventDefaultForCurrentTarget, children: jsxRuntime.jsx(Text.Text, { variant: "primary", children: translations.select_noMatchesFound() }) }));
}
...
function isSearchClearedFirstTime(prevSearchText, searchText) {
    return prevSearchText === undefined && searchText === '';
}
const LEGACY_IME_PROCESS_KEY_CODE = 229;
/**
 * JET-77501 - WHILE TYPING /SEARCHING JAPANESE CHARACTERS THE MENU DID NOT SHOW THE SUGGESTIONS FOR THE ENTERED VALUE
 * `event.isComposing` means the key event is still inside a composition session:
 * https://developer.mozilla.org/en-US/docs/Web/API/KeyboardEvent/isComposing
 * Common CJK IMEs compose text before commit. Apple documents Japanese conversion using
 * Space, arrow keys, and Return; Chinese/Cantonese candidate navigation using arrow keys
 * and Space or Return; and Korean Hanja candidates using arrow keys, Space, and Return.
 * Windows also exposes IME-specific virtual keys, including IME convert and accept keys.
 * https://support.apple.com/guide/japanese-input-method/keyboard-shortcuts-jpim10263/mac
 * https://support.apple.com/guide/chinese-input-method/use-the-candidate-window-cim12992/mac
 * https://support.apple.com/guide/korean-input-method/welcome/mac
 * https://learn.microsoft.com/en-us/windows/win32/inputdev/virtual-key-codes
 * If Select handles these keydown events while composing, it interferes with IME-owned
 * navigation and confirmation. Enter/NumpadEnter can be consumed by Select to commit or clear
 * the select value before the IME commits the composed text. Up/Down can be consumed by Select
 * to prevent default browser/IME behavior or move focus from the input into the select dropdown
 * while the user is still navigating IME candidates.
 */
function shouldIgnoreSelectInputKeyDownDuringComposition(event) {
    const isEnter = event.code === keyboardUtils.KEYS.ENTER || event.code === keyboardUtils.KEYS.NUMPAD_ENTER;
    const isArrow = event.code === keyboardUtils.KEYS.UP || event.code === keyboardUtils.KEYS.DOWN;
    // JET-77501 - WHILE TYPING /SEARCHING JAPANESE CHARACTERS THE MENU DID NOT SHOW THE SUGGESTIONS FOR THE ENTERED VALUE
    // Safari/WebKit requires an additional fallback for the Enter key that confirms an IME
    // candidate. Affected versions dispatch `compositionend` before that `keydown`, so the Enter
    // event incorrectly has `isComposing === false` and would otherwise reach Select's commit logic.
    // Those versions report `keyCode === 229` for the IME-owned event. Although `keyCode` is
    // deprecated, it is intentionally used only for this standards-API false negative:
    // https://developer.mozilla.org/en-US/docs/Web/API/KeyboardEvent/keyCode
    // https://bugs.webkit.org/show_bug.cgi?id=165004
    // WebKit records the event-ordering issue as fixed by the following bug, but the incorrect
    // behavior was still manually reproducible in Safari 26.5. A WebKit source change or backport
    // does not establish that a particular Safari release contains the effective fix. Remove this
    // fallback only after the behavior has been verified in every supported Safari version:
    // https://bugs.webkit.org/show_bug.cgi?id=311717
    // Do not gate the fallback using browser detection. Browser branding does not reliably identify
    // every WebKit host, including alternate browsers on iOS and embedded WKWebView applications.
    // In any browser, key code 229 identifies an IME-processed event, so Select should not treat that
    // Enter as a request to commit or clear its value.
    const isLegacyImeConfirmation = !event.isComposing && isEnter && event.keyCode === LEGACY_IME_PROCESS_KEY_CODE;
    return (event.isComposing && (isEnter || isArrow)) || isLegacyImeConfirmation;
}
/**
 * JET-77501 - WHILE TYPING /SEARCHING JAPANESE CHARACTERS THE MENU DID NOT SHOW THE SUGGESTIONS FOR THE ENTERED VALUE
 * `event.isComposing` means the key event is still inside a composition session:
 * https://developer.mozilla.org/en-US/docs/Web/API/KeyboardEvent/isComposing
 * Common CJK IMEs compose text before commit. Apple documents Japanese conversion using
 * Up/Down Arrow for kanji candidates, Chinese/Cantonese candidate navigation using arrow
 * keys, and Korean Hanja candidate navigation using arrow keys.
 * Windows also exposes IME-specific virtual keys, including IME convert and accept keys.
 * https://support.apple.com/guide/japanese-input-method/keyboard-shortcuts-jpim10263/mac
 * https://support.apple.com/guide/chinese-input-method/use-the-candidate-window-cim12992/mac
 * https://support.apple.com/guide/korean-input-method/welcome/mac
 * https://learn.microsoft.com/en-us/windows/win32/inputdev/virtual-key-codes
 * If Select handles these keyup events while composing, it interferes with IME-owned
 * candidate navigation. Up/Down can open the select dropdown, move focus into the dropdown,
 * or change Select's current row after the IME has used the same key to navigate candidates.
 */
function shouldIgnoreSelectInputKeyUpDuringComposition(event) {
    return event.isComposing && (event.code === keyboardUtils.KEYS.UP || event.code === keyboardUtils.KEYS.DOWN);
}
/**
...
exports.isSearchClearedFirstTime = isSearchClearedFirstTime;
...
exports.shouldIgnoreSelectInputKeyDownDuringComposition = shouldIgnoreSelectInputKeyDownDuringComposition;
exports.shouldIgnoreSelectInputKeyUpDuringComposition = shouldIgnoreSelectInputKeyUpDuringComposition;
exports.useCollectionRenderer = useCollectionRenderer;
exports.useSelectCommon = useSelectCommon;
//# sourceMappingURL=useSelectCommon-73e8b3ab.js.map
