 ...
 */
function useSelection(keyExtractor, selectedKeys, selectionMode, isSelectionRequired, selectionBehavior, onChange, anchorKey, currentKey, getVerticalPrevNextKey, getHorizontalPrevNextKey, scrollToKey, onRangeChange) {
    ...
    const pendingKeyboardSelectionCursor = hooks.useRef(false);
    ...
    const onClickCapture = hooks.useCallback((event) => {
        if (!textSelectionUtils.getIsSelectionPending()) {
            if (!event.shiftKey) {
                // reset keyboard cursor during capture phase, because Selector doesn't propagate click event
                keyboardSelectionCursor.current = undefined;
                pendingKeyboardSelectionCursor.current = false;
            }
            else {
                pendingKeyboardSelectionCursor.current = true;
            }
        }
    }, []);
    ...
    const onKeyDown = hooks.useCallback((event) => {
        ...
        if (pendingKeyboardSelectionCursor.current) {
            keyboardSelectionCursor.current = undefined;
            pendingKeyboardSelectionCursor.current = false;
        }
        ...
        if (Object.keys(keyHandlers).includes(event.key)) {
            // check ctrl/meta keys to avoid conflict gesture with reorder, and reset the range
            if (event.shiftKey &&
                ...
                if (!keys.isKeyDefined(keyboardSelectionCursor.current) && keys.isKeyDefined(currentKey)) {
    ...
    const selectionProps = selectionMode === 'none' || onChange == null
        ? {}
        : { onPointerDown, onClick, onClickCapture, onKeyDown, onKeyUp };
    return { selectionProps };
}

exports.useSelection = useSelection;
//# sourceMappingURL=useSelection-0ef0ee3c.js.map
