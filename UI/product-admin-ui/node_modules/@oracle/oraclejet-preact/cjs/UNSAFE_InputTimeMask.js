...
var timeUtils = require('./timeUtils-cdd5d112.js');
...
var UNSAFE_InputTimeMask_themes_SegmentStyles_css = require('./UNSAFE_InputTimeMask/themes/SegmentStyles.css.js');
require('./getFormatParse-a9299609.js');
...
require('./calendarDateUtils-7652cd25.js');
...
const LiteralSegment = ({ isHidden, isHighlighted, isPlaceholder, text }) => {
    ...
    const spanClasses = classNames.classNames([
        UNSAFE_InputTimeMask_themes_SegmentStyles_css.segmentStyles.literalBase,
        isPlaceholder && UNSAFE_InputTimeMask_themes_SegmentStyles_css.segmentStyles.placeholder,
        isTextOnlyWhitespace && UNSAFE_InputTimeMask_themes_SegmentStyles_css.segmentStyles.space,
        isHighlighted && UNSAFE_InputTimeMask_themes_SegmentStyles_css.segmentStyles.highlighted,
        isHidden ? UNSAFE_InputTimeMask_themes_SegmentStyles_css.segmentStyles.hidden : UNSAFE_InputTimeMask_themes_SegmentStyles_css.segmentStyles.notHidden
    ]);
    return (jsxRuntime.jsx("span", { "aria-hidden": "true", class: spanClasses, children: isTextOnlyWhitespace ? ' ' : text }));
};
...
function useDayPeriodInputHandlers(amPm, ref, onChange) {
    ...
    const compositionRef = hooks.useRef(null);
    const updateDayPeriodSegmentState = hooks.useCallback((value) => {
        onChange?.({
    ...
    const toggleDayPeriodSegmentState = hooks.useCallback(() => {
        onChange?.({
            actionType: 'step',
            data: { type: 'dayPeriod', direction: 'increase' }
        });
    }, [onChange]);
    const changeDayPeriodIfMatch = hooks.useCallback((data) => {
        if (data !== null && data.trim() !== '' && isLetterOnlyString(data)) {
            ...
            const matchType = timeUtils.getAmPmMatchType(amPm, data);
            if (matchType === 'am' || matchType === 'pm') {
                const value = matchType === 'am' ? timeUtils.AM_VALUE : timeUtils.PM_VALUE;
    ...
    const handleBeforeInput = hooks.useCallback((e) => {
        e.preventDefault();
        switch (e.inputType) {
    ...
    const handleInput = hooks.useCallback((e) => {
 ...
 */
const TimeSegment = ({ amPm, 'aria-label': ariaLabel, 'aria-valuemax': ariaValueMax, 'aria-valuemin': ariaValueMin, 'aria-valuenow': ariaValueNow, 'aria-valuetext': ariaValueText, inputRef, isDisabled, isFieldFocused, isHidden, isHighlighted, isInvalid, isRequired, isSelected, onChange, placeholder, text, type }) => {
    const hasDisplayValue = text && text.length > 0;
    const rootClasses = classNames.classNames([
        UNSAFE_InputTimeMask_themes_SegmentStyles_css.segmentStyles.base,
        isSelected && UNSAFE_InputTimeMask_themes_SegmentStyles_css.segmentStyles.selected,
        isHighlighted && UNSAFE_InputTimeMask_themes_SegmentStyles_css.segmentStyles.highlighted,
        isFieldFocused && UNSAFE_InputTimeMask_themes_SegmentStyles_css.segmentStyles.accessibleTouchTarget
    ]);
    const placeholderClasses = classNames.classNames([
        UNSAFE_InputTimeMask_themes_SegmentStyles_css.segmentStyles.placeholder,
        isHidden || hasDisplayValue ? UNSAFE_InputTimeMask_themes_SegmentStyles_css.segmentStyles.hidden : UNSAFE_InputTimeMask_themes_SegmentStyles_css.segmentStyles.notHidden
    ]);
 ...
 */
const useInputMaskSelectFirstHandler = ({ dispatch, isInputFocused }) => {
    ...
    if (isInputFocused && !lastInputFocusedRef.current && !recentPointerRef.current) {
        const action = { actionType: 'selectFirst' };
 ...
 */
const useTimeFieldHandlers = ({ direction, dispatch, groupRef, isDisabled, isInputFocused, state }) => {
    const { tabbableModeProps: { tabIndex: tabbableModeIndex } } = useTabbableMode.useTabbableMode();
    ...
    const timeClearedRef = hooks.useRef(false);
...
const timeReducer = (state, action) => {
    ...
    switch (action.actionType) {
        ...
        case 'goToEnd': {
            const updatedSegment = goToEnd(action.data.type, findSegment$1(state.segments, action.data.type));
            return createTimeFieldStateFromUpdatedSegment(state, action.data.type, updatedSegment, false);
        }
        case 'goToHome': {
            const updatedSegment = goToHome(action.data.type, findSegment$1(state.segments, action.data.type));
            return createTimeFieldStateFromUpdatedSegment(state, action.data.type, updatedSegment, false);
        }
        case 'page': {
            ...
            if (direction === undefined)
                return state;
            const updatedSegment = doStepOrPage(action.data.type, findSegment$1(state.segments, action.data.type), 'page', direction);
            return createTimeFieldStateFromUpdatedSegment(state, action.data.type, updatedSegment, false);
        }
        case 'selectTime': {
        ...
        case 'step': {
            ...
            if (direction === undefined)
                return state;
            const updatedSegment = doStepOrPage(action.data.type, findSegment$1(state.segments, action.data.type), 'step', direction);
            return createTimeFieldStateFromUpdatedSegment(state, action.data.type, updatedSegment, false);
        }
        case 'updateDayPeriodSegment': {
            if (action.data.type === 'dayPeriod') {
                ...
                if (newValue === PM_VALUE || newValue === AM_VALUE) {
                    ...
                    const updatedSegment = createSegmentFromKeyboardAction(action.data.type, newValue, findSegment$1(state.segments, action.data.type));
                    return createTimeFieldStateFromUpdatedSegment(state, action.data.type, updatedSegment, false);
                }
            }
            return state;
        ...
        case 'updateSegment': {
            ...
            if (action.data?.text === '0' &&
                ...
                return createTimeFieldStateFromUpdatedSegment(state, action.data.type, updatedSegment, false, false);
            }
...
const updateSegment = (segmentType, segmentState, action) => {
    ...
    const inRange = maskUtils.withinRange(maskUtils.stringToNumber(requestedValStr), minValue, maxValue);
    let finalValStr = inRange
        ? requestedValStr
        : hasLeadingZeros
            ? actionText.padStart(targetDigitCount, '0')
            : actionText;
    const finalVal = maskUtils.stringToNumber(finalValStr);
...
const createTimeFieldStateFromUpdatedSegments = (currentState, updatedSegments, isTimeSelected) => {
    ...
    if (timeBeforeUpdate !== undefined && timeAfterUpdate !== undefined) {
        ...
        const hasExtraSegments = timeUtils.hasSegmentsNotDisplayed(orderedSegmentsFromCurrentState, timeBeforeUpdate);
        if (hasExtraSegments) {
            useUpdatedValue = timeUtils.displayedTimeSegmentsAreDifferent(orderedSegmentsFromCurrentState, timeBeforeUpdate, timeAfterUpdate);
        }
    }
...
const getAriaValueText = (newVal, segmentState, segmentType, padZero) => {
    ...
    const valStr = maskUtils.numberToString(newVal);
    if (!padZero) {
        return maskUtils.numberToString(newVal);
    }
    const targetDigitCount = segmentType === 'millisecond' ? 3 : 2;
 ...
 **/
const getTimeFieldInitialState = (params) => {
    ...
    const hourValueStr = hasHour
        ? timeUtils.getHourValueStr(currentValue.hour, hasDayPeriod, hasLeadingZeroForHour)
        : undefined;
    const hourValue = hourValueStr !== undefined ? maskUtils.stringToNumber(hourValueStr) : undefined;
    const dayPeriodValueStr = hasDayPeriod && hasHour ? timeUtils.getDayPeriodValueStr(locale, currentValue.hour) : undefined;
    const dayPeriodValue = dayPeriodValueStr !== undefined ? timeUtils.getDayPeriodValueFromHour(currentValue.hour) : undefined;
    const minuteValueStr = currentValue?.minute === undefined ? undefined : timeUtils.padWithZero(currentValue.minute, 2);
    const secondValueStr = currentValue?.second === undefined ? undefined : timeUtils.padWithZero(currentValue.second, 2);
    const millisecondValueStr = currentValue?.millisecond === undefined ? undefined : timeUtils.padWithZero(currentValue.millisecond, 3);
    ...
    const dayPeriodSeg = [
        {
            type: 'dayPeriod',
            amPm: timeUtils.getAmPmStringsForLocale(locale),
            'aria-label': ariaLabels.dayPeriod,
            'aria-valuemin': timeUtils.AM_VALUE,
            'aria-valuemax': timeUtils.PM_VALUE,
 ...
 */
const useTimeAriaLabels = ({ locale }) => {
    const { am, pm } = hooks.useMemo(() => {
        return timeUtils.getAmPmStringsForLocale(locale);
    }, [locale]);
    const translations = useTranslationBundle.useTranslationBundle('@oracle/oraclejet-preact');
    const ariaLabels = {
 ...
 *
 */
const useTimeReset = ({ dispatch, granularity, hasLeadingZeroForHour, locale, state, timeMasks, value: currentValue }) => {
    ...
    const timeResetRef = hooks.useRef(false);
    ...
    if (timeUtils.timesAreDifferent(lastValueFromParentRef.current, currentValue)) {
        lastValueFromParentRef.current = currentValue;
        if (timeUtils.timesAreDifferent(currentValue, state.currentTime)) {
 ...
 */
const useTimeFieldCommitInput = ({ timeResetRef, dispatch, onCommit, onInput, state, value: currentValue }) => {
    // Keep track of the last changed and last committed time values.
    const lastTimeRef = hooks.useRef(state.currentTime);
    const lastCommittedTimeRef = hooks.useRef(state.currentTime);
    ...
    const doCommitIfNeeded = hooks.useCallback(() => {
        if (timeUtils.timesAreDifferent(lastCommittedTimeRef.current, state.currentTime)) {
    ...
    hooks.useLayoutEffect(() => {
        ...
        if (timeUtils.timesAreDifferent(lastTimeRef.current, state.currentTime) && !timeResetRef.current) {
            const frozenTime = state.currentTime !== undefined
 ...
 */
const TimeField = ({ 'aria-describedby': describedBy, 'aria-invalid': ariaInvalid, 'aria-label': ariaLabel, 'aria-labelledby': ariaLabelledBy, baseId, dispatch, fieldRef, granularity = 'minute', hasLeadingZeroForHour, hasInsideLabel, isRequired, masks: timeMasks, onBlur, onCommit, onFocus, onInput, state, testId, textAlign, value, variant = 'default' }) => {
    ...
    const formattedTimeStr = !state.isCompleteTime
        ? ''
        : timeUtils.formatTimeFromMasks(locale, state.currentTime, granularity, timeMasks, hasLeadingZeroForHour);
    const formattedTimeAriaId = `${baseId}-time`;
    const formattedTimeAria = state.isCompleteTime ? (jsxRuntime.jsx(HiddenAccessible.HiddenAccessible, { id: formattedTimeAriaId, children: formattedTimeStr })) : undefined;
    const ariaDescribedBy = state.isCompleteTime
        ? stringUtils.l([describedBy, formattedTimeAriaId])
        : describedBy;
    ...
    return (jsxRuntime.jsxs("div", { "aria-describedby": ariaDescribedBy, "aria-label": ariaLabel, "aria-labelledby": ariaLabelledBy, class: compClasses, onFocusIn: onFocus, onFocusOut: onBlur, ref: groupRef, role: "group", tabIndex: tabIndex, ...timeFieldHandlers, children: [jsxRuntime.jsx(Flex.Flex, { direction: "row", wrap: "nowrap", justify: textAlign, height: "100%", align: "center", children: timeMasks.map(({ type, value: placeholder }) => {
                    if (type === 'literal') {
                        return (jsxRuntime.jsx(LiteralSegment, { isHidden: isHidden, isHighlighted: isTimeSelected, isPlaceholder: !isPartialTime, text: placeholder }));
                    }
                    else if (segmentsMap[type] !== undefined) {
                        const segmentProps = segmentsMap[type];
                        return (jsxRuntime.jsx(TimeSegment, { inputRef: type === firstSegmentType ? fieldRef : undefined, isDisabled: isDisabled, isFieldFocused: isFocused, isInvalid: !!ariaInvalid, isHidden: isHidden, isHighlighted: isTimeSelected, isRequired: isRequired, placeholder: placeholder, ...segmentHandlers, ...segmentProps }));
                    }
                    return null;
                }) }), formattedTimeAria, !isDisabled && jsxRuntime.jsx(LiveRegion.LiveRegion, { children: announceTimeClearedText }), !isDisabled && (jsxRuntime.jsx(LiveRegion.LiveRegion, { testId: testId ? testId + '_autoFilledTime' : undefined, children: announceAutoFilledTime }))] }));
};
 ...
 */
const useTimeFieldState = ({ granularity, hasLeadingZeroForHour, locale, timeMasks, value: currentValue }) => {
    const ariaLabels = useTimeAriaLabels({ locale });
    const [state, dispatch] = hooks.useReducer(timeReducer, {
        ariaLabels,
        currentValue,
        hasLeadingZeroForHour,
        granularity,
...
const InputTimeMask = compat.forwardRef(({ 'aria-describedby': ariaDescribedBy, assistiveText, columnSpan, endContent, granularity = 'minute', helpSourceLink, helpSourceText, hourClock = 'fromLocale', isDisabled: propIsDisabled, isReadonly: propIsReadonly, isRequired, isRequiredShown, label, labelEdge: propLabelEdge, labelStartWidth: propLabelStartWidth, leadingZeroForHour = 'fromLocale', mainFieldRef, messages, masks, testId, textAlign: propTextAlign, userAssistanceDensity: propUserAssistanceDensity, value, variant, onCommit, onInput }, ref) => {
    ...
    const timeMasks = hooks.useMemo(() => {
        ...
        const placeholderMasks = masks
            ? timeUtils.replaceCustomMasksValuesWithPlaceholders(locale, masks) // no need to pass in hourClock. if masks has dayPeriod, then we use it since we checked before this if mask has dayPeriod and hour 12 is explicitly set to false, we throw an error.
            : // we get masks from locale.
                ...
                timeUtils.getTimeMasksFromLocaleAndOptions(locale, granularity, hour12);
    ...
    const resolvedHasLeadingZeroForHour = leadingZeroForHour === 'fromLocale'
        ? timeUtils.getLeadingZeroForHour(locale)
    ...
    const onTimeFieldFocusOut = hooks.useCallback((event) => {
    ...
    const { onBlur: handleTimeFieldBlur } = hooks.useMemo(() => {
        return mergeProps.mergeProps({ onBlur: onTimeFieldFocusOut }, { onBlur: inputPropsOnBlur });
    }, [inputPropsOnBlur, onTimeFieldFocusOut]);
    if (isReadonly) {
        return (jsxRuntime.jsx(FormFieldContext.FormFieldContext.Provider, { value: formFieldContext, children: jsxRuntime.jsx(StyledTextField.ReadonlyTextField, { role: "presentation", columnSpan: columnSpan, compactUserAssistance: compactUserAssistance, inlineUserAssistance: inlineUserAssistance, onBlur: focusProps.onFocusOut, onFocus: focusProps.onFocusIn, ref: anchorRef, testId: testId, mainContent: jsxRuntime.jsx(ReadonlyTextFieldInput.ReadonlyTextFieldInput, { "aria-describedby": timeFieldProps['aria-describedby'], "aria-label": ariaLabel, "aria-labelledby": labelProps.id, as: "div", elementRef: readonlyElementRef, textAlign: textAlign, value: value === undefined
                        ? ''
                        : timeUtils.formatTimeFromMasks(locale, value, granularity, timeMasks, resolvedHasLeadingZeroForHour), hasEmptyLabel: label === '' && labelEdge === 'none', hasInsideLabel: hasInsideLabel, onBlur: inputProps.onBlur, onFocus: inputProps.onFocus }), ...fieldLabelProps }) }));
    }
    ...
    const mainContent = (jsxRuntime.jsx(TimeField, { "aria-label": ariaLabel, "aria-labelledby": ariaLabelledBy, baseId: baseId, dispatch: dispatch, fieldRef: enabledElementRef, granularity: granularity, hasInsideLabel: labelComp !== undefined && labelEdge === 'inside', hasLeadingZeroForHour: resolvedHasLeadingZeroForHour, isRequired: isRequired, masks: timeMasks, onBlur: handleTimeFieldBlur, onCommit: onCommit, onInput: onInput, state: state, testId: testId ? testId + '_timeField' : undefined, textAlign: textAlign, value: value, ...timeFieldProps }));
...
exports.InputTimeMask = InputTimeMask;
