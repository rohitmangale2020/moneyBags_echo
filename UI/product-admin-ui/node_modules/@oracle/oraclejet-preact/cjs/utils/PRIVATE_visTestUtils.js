...
function checkStringContent(content) {
    const targetElements = document.querySelectorAll('[role="tooltip"]');
    const targetElementItem = targetElements.item(0);
    chai.expect(Array.from(targetElementItem.classList).some((className) => className.includes('useDatatip_visDatatipStyle'))).to.equal(true);
    chai.expect(targetElementItem?.innerHTML).to.equal(content);
}
...
/**
 * Populates 'x', 'y', 'clientX', 'clientY', `pageX`, `pageY`, `offsetX`, and `offsetY` properties for a MouseEvent.
 * @param x
 * @param y
 */
function getEventCoords(x, y) {
    return {
        x,
        y,
        clientX: x,
...
exports.checkStringContent = checkStringContent;
