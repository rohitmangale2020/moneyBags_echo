...
var useRadioFocusManager = require('./useRadioFocusManager-58e51026.js');
...
var CheckboxRadioField = require('./CheckboxRadioField-13d71c60.js');
...
require('./CheckboxRadioLabelValueLayout-a4908590.js');
...
const RadioSet = compat.forwardRef(({ 'aria-describedby': ariaDescribedBy, assistiveText, columnSpan, direction = 'column', helpSourceLink, helpSourceText, isDisabled: propIsDisabled, isReadonly: propIsReadonly, isRequired = false, label, labelEdge: propLabelEdge, labelStartWidth: propLabelStartWidth, messages, onCommit, testId, userAssistanceDensity: propUserAssistanceDensity, value, children }, ref) => {
    ...
    // When in readonly mode, we need to navigate the readonly divs using arrows keys to mimic default browser behavior for radiosets
    const containerRef = compat.useRef(null);
    const { setFocusIndex, handleArrowKeyPress } = useRadioFocusManager.useRadioFocusManager(containerRef, children);
    const { focusProps, isFocused } = useFocusWithin.useFocusWithin({
        isDisabled
    });
    const { isTabbable } = useTabbableMode.useTabbableMode();
    return (jsxRuntime.jsx(FormFieldContext.FormFieldContext.Provider, { value: formFieldContext, children: jsxRuntime.jsx(TabbableModeContext.TabbableModeContext.Provider, { value: { isTabbable: isFocused && isReadonly ? false : isTabbable }, children: jsxRuntime.jsx(CheckboxRadioField.CheckboxRadioField, { "aria-describedby": ariaDescribedBy, assistiveText: assistiveText, baseId: baseId, columnSpan: columnSpan, helpSourceLink: helpSourceLink, helpSourceText: helpSourceText, isDisabled: isDisabled, isReadonly: isReadonly, isRequired: isRequired, label: label, labelEdge: labelEdge, labelStartWidth: labelStartWidth, parentComponentVariant: "radioSet", messages: messages, userAssistanceDensity: userAssistanceDensity, hasValue: hasValue, ref: ref, role: "radiogroup", testId: testId, children: jsxRuntime.jsx("div", { ref: containerRef, onKeyDown: isReadonly ? handleArrowKeyPress : undefined, ...(isReadonly ? focusProps : {}), children: jsxRuntime.jsx(RadioItemContext.RadioSetContext.Provider, { value: {
                            name: baseId,
                            value,
                            onCommit,
                            setFocusIndex
                        }, children: jsxRuntime.jsx(Flex.Flex, { direction: direction, ...(direction === 'row' && { gap: [0, utils.xUnits(8)], wrap: 'wrap' }), children: preact.toChildArray(children).map((child, index) => (jsxRuntime.jsx(RadioItemContext.RadioItemContext.Provider, { value: { index }, children: child }))) }) }) }) }) }) }));
});
