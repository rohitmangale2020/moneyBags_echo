...
const useNavigation = (data, rootRef, idExtractor, itemSelector, arrowKeyAndHandlers, currentItemOverride, onPersistCurrentItem, viewportConfig, scrollToVisibleOffset, allowShiftKey, allowTabbableMode, allowActiveDescendant, isEmbedded, allowEnterKey) => {
    ...
    const [isCurrentTabbableKey, tabbableModeProps] = useTabbableModeSet.useTabbableModeSet(rootRef, (element) => {
        ...
        return isExpander(element) ? null : collectionUtils.keyExtractor(element, itemSelector);
    }, currentKey, onCurrentKeyChange, undefined, undefined, allowEnterKey);
    const { currentKeyProps } = useCurrentKey.useCurrentKey((element) => collectionUtils.keyExtractor(element, itemSelector), allowShiftKey, isCurrentTabbableKey(currentKey), arrowKeyAndHandlers.ArrowUp(currentKey), arrowKeyAndHandlers.ArrowDown(currentKey), arrowKeyAndHandlers.ArrowLeft?.(currentKey), arrowKeyAndHandlers.ArrowRight?.(currentKey), currentKey, onCurrentKeyChange);
    ...
    const handleInvalidCurrentKey = hooks.useCallback(() => {
        if (rootRef.current && (!allowTabbableMode || isCurrentTabbableKey(undefined))) {
            const firstKey = pendingCurrentKey.current || collectionUtils.getFirstVisibleKey(rootRef.current, itemSelector);
    ...
    const handleFocus = hooks.useCallback((event) => {
        ...
        if (rootRef.current) {
            if (!keys.isKeyDefined(currentKey)) {
                ...
                const elem = collectionUtils.findElementByKey(rootRef.current, currentKey, itemSelector);
                if (elem) {
...
exports.LIST_ITEM_SELECTOR = LIST_ITEM_SELECTOR;
...
//# sourceMappingURL=useNavigation-54122cfc.js.map
