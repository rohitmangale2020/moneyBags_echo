...
var size = require('./size-1914356b.js');
...
var TimeComponentCanvas2D = require('./TimeComponentCanvas2D-ec452463.js');
var useUser = require('./useUser-cc045b3e.js');
var Layer = require('./Layer-06e11b9c.js');
require('preact/compat');
require('./LayerHost-21391a0d.js');
var Floating = require('./Floating-381eedb9.js');
...
var utils = require('./utils-c0efcd5e.js');
...
function GanttRowLabelContainer({ rowObj, rowData, isRowSelected, isFocused, isHorizontalGridlinesVisible, ariaActive, children }) {
    ...
    return (jsxRuntime.jsx("div", { id: ariaActive?.dataId === rowObj.id ? ariaActive.ariaId : undefined, "data-oj-private-row-index": rowObj.index, role: "row", "aria-label": ariaLabel, class: classNames.classNames([
 ...
 */
function useDatatip({ content, placement = 'top-end-corner', offset, borderColor, anchor, targetFocusRef, defaultContainer = 'enabled' }) {
    ...
    const datatipContent = content != null && content != '' ? (jsxRuntime.jsx(Layer.Layer, { children: jsxRuntime.jsx(Floating.Floating, { anchorRef: anchorRef, placement: placement, offsetValue: offsetValue, flipOptions: { crossAxis: true, mainAxis: true }, shiftOptions: { crossAxis: false, mainAxis: false }, children: jsxRuntime.jsx("div", { id: uniqueIdRef.current, className: classNames.classNames([
                    hooks_PRIVATE_useDatatip_themes_useDatatip_css.styles.visDatatipStyle,
                    defaultContainer === 'enabled' ? hooks_PRIVATE_useDatatip_themes_useDatatip_css.styles.borderEnabled : undefined
                ]), style: { borderColor }, children: content }) }) })) : null;
    return {
        datatipContent,
        datatipProps: {
...
const useGanttTaskDatatip = ({ targetRef, datatip, rowData, dateFormatter, hoveredTaskObj, focusedTaskObj, currentTaskInteraction, dataBodyRegion }) => {
    ...
    const context = activeTaskObj
        ? getRenderContext(rowData, activeTaskObj)
        : undefined;
    if (context) {
        const config = datatip?.(context);
        const defaultRenderer = getDefaultDatatipRenderer({
            translations,
            dateFormatter,
            isRTL
        });
        const defaultConfig = defaultRenderer(context);
        content = config?.content === '' ? undefined : config?.content || defaultConfig.content;
        borderColor = config?.borderColor || defaultConfig.borderColor;
        defaultContainer = config?.defaultContainer || defaultConfig.defaultContainer;
    }
...
const drawTaskEffectRing = (ctx, effectLayout) => {
    ...
    const effectRingCmd = lineUtils.rectangleWithBorderRadius(ringX, ringY, ringWidth, ringHeight, borderRadius, Math.min(ringWidth, ringHeight), '0');
...
const drawTask = (ctx, taskLayoutObj, state) => {
    ...
    const roundRectCmd = lineUtils.rectangleWithBorderRadius(x, y, width, height, borderRadius, Math.min(width, height), '0');
...
const timeComponentUnresolvedVars = [
    ...
    {
        tokenVar: themeContract_css.typographyVars.body.md.fontSize,
        key: 'majorAxisFontSize',
        cssProp: 'font-size'
    },
    {
        tokenVar: themeContract_css.typographyVars.body.sm.fontSize,
        key: 'minorAxisFontSize',
        cssProp: 'margin-block-start'
    },
    {
        tokenVar: themeContract_css.typographyVars.body.md.fontWeight.semibold,
        key: 'majorAxisFontWeight'
    },
    {
        tokenVar: themeContract_css.typographyVars.body.sm.fontWeight.regular,
...
const ganttUnresolvedVars = [
    ...
    {
        tokenVar: themeContract_css.baseVars.boxShadow.md,
