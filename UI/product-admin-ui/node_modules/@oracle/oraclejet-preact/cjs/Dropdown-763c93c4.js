...
var usePopupAnimation = require('./usePopupAnimation-985e90a2.js');
...
var useOutsideMousedown = require('./useOutsideMousedown-eda95637.js');
...
/**
 * Functions to get type of device
 */
const getIsMobile = () => {
    const { deviceRenderMode } = clientHints.getClientHints();
    return deviceRenderMode === 'phone';
};
const getIsIosTablet = () => {
    const { deviceRenderMode, platform } = clientHints.getClientHints();
    return platform === 'ios' && deviceRenderMode === 'tablet';
};
/**
...
const isMenuChildrenFalsy = (children) => {
...
function Dropdown({ isOpen = false, anchorRef, children, id, onClose, minWidth, placement = DEFAULT_PLACEMENT, offset = DEFAULT_OFFSET, onPosition, flipOptions = DEFAULT_FLIP_OPTIONS, sizeOptions = DEFAULT_SIZE_OPTIONS, shiftOptions, initialFocus = 'none', onTransitionEnd }) {
    const dropdownRef = hooks.useRef(null);
    const _onTransitionEnd = hooks.useCallback((isOpen) => {
        if (!isOpen) {
        ...
        // honor firstFocusable initial focus behavior
        if (initialFocus === 'firstFocusable') {
            tabbableUtils.focusWithin(dropdownElement);
            onTransitionEnd?.(isOpen);
            return;
        }
        // JET-76442 + JET-76109: When VoiceOver is enabled on iOS tablets, we want the first menu item to receive focus.
        // This is to ensure that VoiceOver announces the menu items correctly and focus does not go to the next tabbable element.
        if (getIsIosTablet()) {
            const firstItem = dropdownElement.querySelector('[data-oj-menu-item-key]');
            if (firstItem) {
                tabbableUtils.focusWithin(firstItem);
            }
        }
        onTransitionEnd?.(isOpen);
    }, [initialFocus]);
    ...
    // useEffectEvent ensures we always access the latest anchorRef.current value inside effects,
    // without having to include anchorRef in the dependency array and the eslint-disable-next comment
    const STABLE_getAnchor = useEffectEvent.useEffectEvent(() => anchorRef?.current);
    // Restore focus to anchor after Dropdown is closed
    hooks.useEffect(() => {
        const anchorElement = STABLE_getAnchor();
        if (!isOpen &&
            status === 'unmounted' &&
            anchorElement &&
    ...
    }, [STABLE_getAnchor, isOpen, status]);
...
exports.getMenuItemClickChangeKey = getMenuItemClickChangeKey;
exports.getMenuPointerDown = getMenuPointerDown;
exports.getNVisible = getNVisible;
exports.getOpenSubmenuEvent = getOpenSubmenuEvent;
exports.getPrevNext = getPrevNext;
exports.isMenuChildrenFalsy = isMenuChildrenFalsy;
exports.keyElemExtractor = keyElemExtractor;
//# sourceMappingURL=Dropdown-763c93c4.js.map
