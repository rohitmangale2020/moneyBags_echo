...
export type LabelledLinkProps = ObservedGlobalProps<'aria-describedby'> & {
    ...
    /**
     * @description
     * Specifies the URL that the link points to. If a safe href is provided, clicking the link
     ...
     * <p><b>Note:</b> This value is rendered as a link
     * <code class="prettyprint">href</code>. Applications are responsible for providing trusted
     * URLs, choosing protocols appropriate for their use case, and enforcing any required origin or
     * host allowlist. Unsafe URL protocols are blocked by omitting the rendered navigable href;
     * blocked href values do not trigger <code class="prettyprint">ojAction</code>.</p>
    ...
    /**
     * @ojmetadata description "Truncates text at a specific number of lines and then displays an ellipsis (…) at the end of the last line. The parent of the element needs to have a specific width so that text starts overflowing and produces a truncation. The lineClamp property is ignored when the oj-c-labelled-link is in a oj-c-form-layout."
     * @ojmetadata displayName "lineClamp"
     * @ojmetadata help "#lineClamp"
     */
    lineClamp?: PreactLabelledLinkProps['lineClamp'];
    /**
     ...
     * or touch events. This event is triggered only when the href is not provided. If a safe href
     * is provided, clicking the link will not trigger this event. If an unsafe href protocol is
     * provided, the navigable href is omitted and this event is not triggered."
    ...
    onOjAction?: Action;
    /**
     * @ojmetadata description "Specifies how the text should be truncated when it overflows.
     * The parent of the element needs to have a specific width so that text starts overflowing and produces a truncation.
     * Truncation should not be used when lineClamp is used. The default behavior is for end truncation to occur, except for mixed readonly links, then truncation is forced to none, which can cause the link to clip."
     * @ojmetadata eventGroup "common"
     * @ojmetadata displayName "truncation"
     * @ojmetadata help "#truncation"
     */
    truncation?: 'none' | 'end' | 'middle';
};
/**
 * @classdesc
...
export namespace CLabelledLinkElement {
    ...
    type lineClampChanged = JetElementCustomEventStrict<CLabelledLinkElement['lineClamp']>;
    ...
    type truncationChanged = JetElementCustomEventStrict<CLabelledLinkElement['truncation']>;
...
export interface CLabelledLinkElementEventMap extends HTMLElementEventMap {
    ...
    'lineClampChanged': JetElementCustomEventStrict<CLabelledLinkElement['lineClamp']>;
    ...
    'truncationChanged': JetElementCustomEventStrict<CLabelledLinkElement['truncation']>;
...
export interface CLabelledLinkElementSettableProperties extends JetSettableProperties {
    ...
    lineClamp?: LabelledLinkProps['lineClamp'];
    ...
    truncation?: LabelledLinkProps['truncation'];
    /**
     * This is temporary unsafe API for JET-52089. It enables a wrapper VComponent to pass
     * the id of an external label that it wants to associate with the inner input.
     */
...
export interface LabelledLinkIntrinsicProps extends Partial<Readonly<CLabelledLinkElementSettableProperties>>, GlobalProps, Pick<preact.JSX.HTMLAttributes, 'ref' | 'key'> {
    ...
    onlineClampChanged?: (value: CLabelledLinkElementEventMap['lineClampChanged']) => void;
    ...
    ontruncationChanged?: (value: CLabelledLinkElementEventMap['truncationChanged']) => void;
